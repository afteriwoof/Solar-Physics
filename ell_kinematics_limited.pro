...
pro ell_kinematics_limited, in, da, fronts, errs, kins, mov, noplot=noplot, noflank=noflank, save_ells=save_ells
...
for k=0,sz[2]-1 do begin
	...
	if keyword_set(noflank) then begin
		front_ell_limited, fronts[*,*,k], errs[k], in[k], da[*,*,k], ang, ell_info, r_err, xe, ye, perror, max_h_front, max_h_ell, apex_a, noplot, /noflank
	endif else begin
		front_ell_limited, fronts[*,*,k], errs[k], in[k], da[*,*,k], ang, ell_info, r_err, xe, ye, perror, max_h_front, max_h_ell, apex_a, noplot
	endelse
