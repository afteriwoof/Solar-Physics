...
pro cme_detection_maskC3, in, da, modgrads, alpgrads, circ_pol, list, cme_detect, show=show
...
; for each scale in the decomp.	
	...
	alp1 = rm_inner(alpgrads[*,*,s], in, dr_px, thr=5)
	alp1 = rm_outer(alp1, in, dr_px, thr=28.5)
	mod1 = rm_inner(modgrads[*,*,s], in, dr_px, thr=5)
	mod1 = rm_outer(mod1, in, dr_px, thr=28.5)
	mod_rm = mod1
	...
	if keyword_set(show) then window, xs=600, ys=600
	...
	for c=0,no_contours-1 do begin
		...
		if med_temp gt 0.20 then begin
			...
			legend, '*** CME DETECTED ***', charsize=2, /clear & wait, 2;, /bottom
		...
		if med_temp gt 0.1 and med_temp le 0.2 then begin
			...
			legend, 'CME and/or streamer?', charsize=2, /clear & wait, 2
		...
		if med_temp gt 0.05 and med_temp le 0.1 then begin
			...
			legend, 'Weak CME? or streamer.', charsize=2, /clear & wait, 2
