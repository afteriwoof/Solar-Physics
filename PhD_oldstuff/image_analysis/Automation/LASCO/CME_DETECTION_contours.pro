...
pro cme_detection_contours, in, da, alpgrads, modgrads, circ_pol, list 
...
cme_detect = fltarr(600,800,no_contours*sz[2])
count = -1
...
for s=0,sz[2]-1 do begin
	...
	for c=0,no_contours-1 do begin
		...
		pause
		range = where(circ_pol[*,c,s] gt 0.35 and circ_pol[*,c,s] lt 0.4)
