...
; for each scale in the decomp.	
	...
	if in.detector eq 'C2' then begin
	...
	if in.detector eq 'C3' then begin
		alp_pol = rm_inner(alpgrads[*,*,s], in, dr_px, thr=4.5)
		alp_pol = rm_outer(alp_pol, in, dr_px, thr=29)
		mod_pol = rm_inner(modgrads[*,*,s], in, dr_px, thr=4.5)
		mod_pol = rm_outer(mod_pol, in, dr_px, thr=29)
	endif
	mod_rm = mod_pol
