...
; Last edited 06-04-07


pro CME_offedge1, req_x_next, req_y_next, final_front_x, final_front_y, x_next, y_next, req_x_front, req_y_front, front_arr_x, front_arr_y, next_front, front_vel, front_arr_r, tangents=tangents, previous=previous, lines=lines
...
;						print, 'count_x:' & print, count_x
			...
			if nonzeros[0] ne -1 then begin
				next_offedge_i = next_offedge_i + 1
				if next_offedge_i eq 1 then next_offedge = i
...
;*****************************************************************
	...
	;		plots, x_mid, y_mid
		...
		; Label the part which gives the first front offedge needed.
		if i eq next_offedge+1 then begin
		req_x_next = x_next[*,*,next_offedge-1]
		req_y_next = y_next[*,*,next_offedge-1]
		;plots, req_x_next, req_y_next, psym=2
		endif
...
;						plot, offedge_x, offedge_y, color=5
				...
				true = 0
				for j=0,sz_tempx[1]-1 do begin
					if abs(front_arr_y[j,k,i+1]) gt 5900. then true = 1
				endfor
					if true eq 1 then begin	
...
;									print, reqx, reqy, x, y
									;***
;									plots, x_next[*, *, i-1], y_next[*, *,i-1], psym=-5, color=2
;									plots, req_x_front[*,*,i+1], req_y_front[*,*,i+1], psym=6, color=6
									;***
...
plot, final_front_x[*,*,next_offedge], final_front_y[*,*,next_offedge], psym=2

plot, x_next[*,0,next_offedge-1], y_next[*,0,next_offedge-1], psym=4, color=3
