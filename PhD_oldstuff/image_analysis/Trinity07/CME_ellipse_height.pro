...
pro CME_ellipse_height, tangents=tangents, previous=previous, lines=lines
...
ellipse_angle = fltarr(szd[2])
ellipse_width = fltarr(szd[2])
...
;		to find the fit given by MPFITFUN for inclined ellipse equation.
	...
	; MPFITELLIPSE	
		...
		if p[0] gt p[1] then ellipse_width[i] = p[0]
		if p[1] gt p[0] then ellipse_width[i] = p[1]
	;print, ellipse_width	
		ellipse_angle[i] = 360/(2*!pi)*p[4]
		
;		print, 'degrees:' & print, (360/(2*!pi))*p(4)
