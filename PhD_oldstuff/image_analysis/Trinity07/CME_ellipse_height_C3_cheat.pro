...
pro CME_ellipse_height_C3_cheat, da_norm, danb, tangents=tangents, previous=previous, lines=lines
...
loadct, 3
plot_map, mapsb[szd[2]-1]

for i=10,szd[2]-1 do begin
...
;****************************************************
		...
		read, 'y/n', ans
		if ans eq 'y' then plots, x_front, y_front
	...
	; MPFITELLIPSE	
		...
		if p[0]	lt 12000 then begin
			if p[1] lt 12000 then begin
				...
				if ans eq 'y' then plots, x_ellipse_new, y_ellipse_new
				if ans eq 'y' then plots, p[2], p[3], psym=2
			endif
		endif
		...
		if ans eq 'y' then plots, [0,max_x], [0,max_y], color=3
		print, max_x, max_y
...
;wr_movie, 'CME_ellipse_C3_cheat_red', temparr[*,*,5:szd[2]-1]
