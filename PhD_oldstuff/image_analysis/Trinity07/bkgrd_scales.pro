...
pro bkgrd_scales, decomps, features, filtered, tangents=tangents, previous=previous, lines=lines
...
;save, da_norm, f='dan_18apr00.sav'
;save, danb, f='danb_240107.sav'

restore, '~/PhD/Data_sav_files/dan_18apr00.sav'
...
;********
; Creating the background image from multiscale analysis

decomps = fltarr(sz[0],sz[1],sz[2],10)
...
for i=0,sz[2]-1 do begin
	features[*,*,i] = da_norm[*,*,i] - (decomps[*,*,i,0]+decomps[*,*,i,1]+decomps[*,*,i,2])
	filtered[*,*,i] = features[*,*,i] - (decomps[*,*,i,9]+decomps[*,*,i,8]+decomps[*,*,i,7])
endfor
...
;*****************************************************************
	...
	;********
		...
		r_sun = pb0r(fmaps[i].time, /soho, /arcsec)
