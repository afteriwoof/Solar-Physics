...
pro mapcontours_fast_9, tangents=tangents, previous=previous, lines=lines
...
;****************************************************
		...
		;*****************
		; Trying to only take the points on the front, rather than any max point since it can be off the map edge!

		;*****
		; Obtaining the points on the front which are closest to the map edges	
		sz_x_front = size(x_front, /dim)
		sz_y_front = size(y_front, /dim)
		close_x_edge1 = fltarr(sz_x_front[0])
		close_x_edge2 = fltarr(sz_x_front[0])
		for j=0,sz_x_front[0]-1 do begin
			close_x_edge1[j] = abs(x_edge1 - x_front[j])
			close_x_edge2[j] = abs(x_edge2 - x_front[j])
		endfor
		close_x_1 = [min(close_x_edge1)]
		close_x_2 = [min(close_x_edge2)]
		closest_x_edge1 = x_front(where(close_x_edge1 eq close_x_1[0]))
		closest_x_edge2 = x_front(where(close_x_edge2 eq close_x_2[0]))
		print, closest_x_edge1, closest_x_edge2
	
		close_y_edge1 = fltarr(sz_y_front[0])
		close_y_edge2 = fltarr(sz_y_front[0])
		for j=0,sz_y_front[0]-1 do begin
			close_y_edge1[j] = abs(y_edge1 - y_front[j])
			close_y_edge2[j] = abs(y_edge2 - y_front[j])
		endfor
		close_y_1 = [min(close_y_edge1)]
		close_y_2 = [min(close_y_edge2)]
		closest_y_edge1 = y_front(where(close_y_edge1 eq close_y_1[0]))
		closest_y_edge2 = y_front(where(close_y_edge2 eq close_y_2[0]))
		print, closest_y_edge1, closest_y_edge2
		;*****

		closest = [closest_x_edge1, closest_x_edge2, closest_y_edge1, closest_y_edge2]
		print, closest
...
;*****************************************************************
;               Using the average point of the front coords (scaled to origin) to find the fit given by MPFITFUN for inclined ellipse equation.
