...
; Uses the atrous algorithm to characterise the CME front.
...
; Last edited 02-04-07


pro CME_ellipse_atrous, decomps, maps, tangents=tangents, previous=previous, lines=lines
...
;***Comment out this block now I've decomps saved.
;decomps = fltarr(sz[0], sz[1], 10, sz[2]-1)
;maps = decomps
...
;endfor
;
;save, decomps, filename='decomps.sav'
;restore, 'decomps.sav'


;***just taking the 3rd scale:
fourths_m = fltarr(sz[0],sz[1],sz[2])
for i=0,sz[2]-1 do begin
	atrous, da_norm[*,*,i], decomp=decomp
	fourth = decomp[*,*,3]
	fourths_m[*,*,i] = abs(deriv(fourth))
endfor
save, fourths_m, filename='fourths_m.sav'
stop
...
; Creating maps but note the header isn't being edited (the times will be wrong)!
	...
	save, maps, filename='maps_atrous.sav'
