...
pro CME_offedge_messin_org, final_front_x, final_front_y, x_next, y_next, req_x_front, req_y_front, front_arr_x, front_arr_y, next_front, front_vel, front_arr_r, tangents=tangents, previous=previous, lines=lines
...
set_line_color
			...
			print, 'unwanted_x' & print, unwanted_x
			print, 	'unwanted_y' & print, unwanted_y
			print, 'x_front' & print, x_front
			print, 'y_front' & print, y_front
			...
			print, 'new_x_front:' & print, new_x_front
			print, 'new_y_front:' & print, new_y_front
...
for i=0,szd[2]-2 do begin
	...
	for k=0,c-1 do begin
		...
		if i ne 0. then begin
			...
			if max(abs(y_next[*,*,i-1])) gt 5900 then true_offedge=1
			if true_offedge eq 1 then begin
			...
			print, 'k i' & print, k & print, i
			plots, relevant_x, relevant_y, psym=-2
			read, 'next?', ans
				;*********
				...
				true1 = 0
				for j=0,sz_tempx[1]-1 do begin
					print, 'front_arr_y' & print, front_arr_y[j,k,i+1]
					print, 'true1 init' & print, true1
					if abs(front_arr_y[j,k,i+1]) gt 5900. then true1 = 1
					print, 'true1 final'  & print, true1
					read, 'front?', ans
				endfor
					if true1 eq 1 then begin	
...
;						plots, x_next[*, 0, i-1], y_next[*, 0,i-1], psym=-5, color=2
						...
						y_next_notoff = y_next[where(abs(y_next[*,k,i-1]) lt 5900),k,i-1]
						x_next_notoff = x_next[where(abs(y_next[*,k,i-1]) lt 5900),k,i-1]
...
;						plots, req_x_front[*,*,i+1], req_y_front[*,*,i+1], psym=6, color=6				
						...
						for m=0,sz_reqx[0]-1 do begin
							if req_x_front[m,n,i+1] ne 0. then begin
								reqx = req_x_front[m,n,i+1]
								reqy = req_y_front[m,n,i+1]
								for l=0,sz_x_next[0]-1 do begin
									x = x_next[l,n,i-1]
									y = y_next[l,n,i-1]
									print, reqx, reqy, x, y
									;***
									plots, x_next[*, n, i-1], y_next[*, n,i-1], psym=-5, color=2
									plots, req_x_front[*,n,i+1], req_y_front[*,n,i+1], psym=6, color=6
									;***
									distance = sqrt((reqx-x)^2+(reqy-y)^2)
									print, 'distance' & print, distance
									if distance lt thresh then true = 1
								endfor
							endif
							...
							plots, final_x, final_y, psym=-2, color=3
						
							print, 'k' & print, k
							print, 'n' & print, n
							print, 'i' & print, i	
							final_front_x[*,n,i] = [final_x]
							final_front_y[*,n,i] = [final_y]
							read, 'y?', ans
						endfor
						endfor
