...
pro CME_offedge, front_arr_x, front_arr_y, next_front, front_vel, front_arr_r, tangents=tangents, previous=previous, lines=lines
...
;endfor
			...
			sz_contours_x = size(contours_x, /dim)
			print, sz_contours_x[0]
			count_x = 0
			count_y = 0
			unwanted_x = fltarr(sz_contours_x[0]+1, c, szd[2])
			unwanted_y = unwanted_x
			help, unwanted_x
			for j=0,sz_contours_x[0]-1 do begin
				if j eq 0 then j_0 = sz_contours_x[0]-1 else j_0 = j-1
				if j eq sz_contours_x[0]-1 then j_n = 0 else j_n = j+1
				if abs(contours_x[j_0, k, i]) gt 5900 then begin
					if abs(contours_x[j_n, k, i]) gt 5900 then begin
						print, 'count_x:' & print, count_x
						unwanted_x[count_x, k, i] = contours_x[j, k, i]
						unwanted_y[count_x, k, i] = contours_y[j, k, i]
						count_x = count_x+1
					endif
				endif
			endfor
			for j=0,sz_contours_x[0]-1 do begin
				if j eq 0 then j_0 = sz_contours_x[0]-1 else j_0 = j-1
				if j eq sz_contours_x[0]-1 then j_n = 0 else j_n = j+1
				if abs(contours_y[j_0, k, i]) gt 5900 then begin
					if abs(contours_y[j_n, k, i]) gt 5900 then begin
						print, 'count_y:' & print, count_y
						unwanted_x[count_y, k, i] = contours_x[j, k, i]
						unwanted_y[count_y, k, i] = contours_y[j, k, i]
						count = count_y+1
					endif
				endif
			endfor

			plot, contours_x[*,k,i], contours_y[*,k,i]
...
;		to find the fit given by MPFITFUN for inclined ellipse equation.
	...
	; Computing the velocities of the front from the difference in the front_arr_r for each image.
		...
		if i ne 0 then begin
			...
			; Condition that once it meets the edge of the image it won't jump into the next front.
			sz_x_next_temp = size(x_next_temp, /dim)
			print, sz_x_next_temp[0]
			count_x = 0
			count_y = 0
			unwanted_x = fltarr(sz_x_next_temp[0]+1)
			unwanted_y = unwanted_x
			help, unwanted_x
			for j=0,sz_x_next_temp[0]-1 do begin
				if j eq 0 then j_0 = sz_x_next_temp[0]-1 else j_0 = j-1
				if j eq sz_x_next_temp[0]-1 then j_n = 0 else j_n = j+1
				if abs(x_next_temp[j_0]) gt 5900 then begin
					if abs(x_next_temp[j_n]) gt 5900 then begin
						print, 'count_x:' & print, count_x
						unwanted_x[count_x] = x_next_temp[j]
						unwanted_y[count_x] = y_next_temp[j]
						count_x = count_x+1
					endif
				endif
			endfor
			for j=0,sz_x_next_temp[0]-1 do begin
				if j eq 0 then j_0 = sz_x_next_temp[0]-1 else j_0 = j-1
				if j eq sz_x_next_temp[0]-1 then j_n = 0 else j_n = j+1
				if abs(y_next_temp[j_0]) gt 5900 then begin
					if abs(y_next_temp[j_n]) gt 5900 then begin
						print, 'count_y:' & print, count_y
						unwanted_x[count_y] = x_next_temp[j]
						unwanted_y[count_y] = y_next_temp[j]
						count = count_y+1
					endif
				endif
			endfor
...
for i=0,szd[2]-2 do begin
	...
	for k=0,c-1 do begin
		...
		if i ne 0. then begin
			...
			if max(abs(y_next[*,*,i-1])) gt 6000 then begin
				...
				plot_map, mapsb[i+1]
				set_line_color
				;plots, relevant_x, relevant_y, psym=2, color=k mod 4
				...
				for j=0,sz_tempx[1]-1 do begin
					if abs(front_arr_x[j,k,i+1]) gt 6000. then begin
					...
					if abs(front_arr_y[j,k,i+1]) gt 6000. then begin
						...
						plots, x_next[*, 0, i-1], y_next[*, 0,i-1], psym=-5, color=2
						plots, offedge_x, offedge_y, psym=-2, color=5
