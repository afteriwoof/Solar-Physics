...
; Last edited 28-03-07


pro CME_offedge_ellipse_redo, final_front_x, final_front_y, x_next, y_next, req_x_front, req_y_front, front_arr_x, front_arr_y, next_front, front_vel, front_arr_r, tangents=tangents, previous=previous, lines=lines
...
set_line_color
			...
			; Condition that once it meets the edge of the image it won't jump into the next inner front.
			count_x = 0
			count_y = 0
...
;			plots, x_next_temp, y_next_temp
	...
	;	read, 'go?', ans
			...
			;Now calculating the ellipse for the predicted next fronts
			
			ellipse_next = ellipsefit(x_next_temp, y_next_temp)
			aprime_next = ellipse_next[0:(size(ellipse_next,/dim)-2)/2.-1]
			bprime_next = ellipse_next[(size(ellipse_next,/dim)-2)/2.:(size(ellipse_next,/dim)-2)-1]
			
			xbar_next = ellipse_next[size(ellipse_next,/dim)-2]
			ybar_next = ellipse_next[size(ellipse_next,/dim)-1]
		
	               
		      	x0_next = x_next_temp - xbar_next
	                y0_next = y_next_temp - ybar_next
			recpol, x0_next, y0_next, y, x
	                radiusbar_next = sqrt(xbar_next^2+ybar_next^2)
			fit = 'sqrt((p(0)^2*p(1)^2)/((p(0)^2+p(1)^2)/2-((p(0)^2-p(1)^2)/2)*cos(2*x-2*p(2))))'
			parinfo_next = replicate({value:0.D, fixed:0, limited:[0,0], limits:[0.D,0]},3)
	                parinfo_next(*).value = [10*width[i,k], 10*width[i,k], 1.]
			param_next = mpfitexpr(fit, x, y, y*0.1, parinfo=parinfo_next, perror=perror_next, yfit=yfit_next)
help, xbar_next, ybar_next
			plots, xbar_next, ybar_next, psym=2
			plots, aprime_next, bprime_next, color=2

	                x_fit_next = x_next_temp
	                y_fit_next = y_next_temp

		; MPFITELLIPSE For the Next Fronts!
								
			start_params_next = [param_next[0],param_next[1],xbar_next,ybar_next,param_next[2]]

			p_next = mpfitellipse(x_fit_next,y_fit_next,start_params_next,/tilt,weights=wts_next)
			;phi = as above

			x_ellipse_next = p_next(2)+p_next(0)*cos(phi)
			y_ellipse_next = p_next(3)+p_next(1)*sin(phi)

			x_ellipse2_next = x_ellipse_next*cos(p_next(4))+y_ellipse_next*sin(p_next(4))
			y_ellipse2_next = y_ellipse_next*cos(p_next(4))-x_ellipse_next*sin(p_next(4))
			
			shift_xbar_next = p_next(2)*cos(p_next(4))+p_next(3)*sin(p_next(4))
			shift_ybar_next = p_next(3)*cos(p_next(4))-p_next(2)*sin(p_next(4))

			shift_factor_x_next = shift_xbar_next - p_next(2)
			shift_factor_y_next = shift_ybar_next - p_next(3)

			x_ellipse_next_new = x_ellipse2_next - shift_factor_x_next
			y_ellipse_next_new = y_ellipse2_next - shift_factor_y_next

			plots, x_ellipse_next_new, y_ellipse_next_new

		; Left off here with some problems with the ellipse fittings!!!!!!!!!!!			
