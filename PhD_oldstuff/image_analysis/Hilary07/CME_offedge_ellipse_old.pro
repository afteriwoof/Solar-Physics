...
pro CME_offedge_ellipse_old, final_front_x, final_front_y, x_next, y_next, req_x_front, req_y_front, front_arr_x, front_arr_y, next_front, front_vel, front_arr_r, tangents=tangents, previous=previous, lines=lines
...
for i=0,szd[2]-2 do begin
	...
	for k=0,c-1 do begin
		...
		print, 'nonzerosx' & print, nonzerosx
		print, 'nonzerosy' & print, nonzerosy
		help, nonzerosx, nonzerosy

;		if nonzerosx ne -1 then print, final_front_x[nonzerosx, k, i ]

;		if nonzerosx ne -1 then x = final_front_x[where(final_front_x[*,k,i] ne 0.), k, i ]
;		if nonzerosy ne -1 then y = final_front_y[where(final_front_y[*,k,i] ne 0.), k, i ]
;		final_xfront[k,i] = [x]
;		final_yfront[k,i] = [y]
