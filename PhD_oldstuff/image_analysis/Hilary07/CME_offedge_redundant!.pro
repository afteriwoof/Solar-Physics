			...
			if max(abs(y_next[*,*,i-1])) gt 6000 then begin
				...
				for j=0,sz_tempx[1]-1 do begin
					...
					if abs(front_arr_y[j,k,i+1]) gt 6000. then begin
						...
						recpol, offedge_x, offedge_y, offedge_r, offedge_a, /degrees
						plot, offedge_r, offedge_a, psym=2
						help, offedge_r
						offedge_r_front = fltarr(sz_tempx[1])
						help, offedge_r_front	
						t = 0
						while (offedge_r[t] lt offedge_r[t+1]) do begin
							print, t
							print, min(offedge_r)
							print, 'offedge_r' & print, offedge_r
							print, [offedge_r[t]]
							offedge_r_front[t] = [offedge_r[t]]
							print, offedge_r_front[t]
							t = t+1
							read, 'alrit?', ans
						endwhile
						;print, 'offedge_r_front:' & print, offedge_r_front
