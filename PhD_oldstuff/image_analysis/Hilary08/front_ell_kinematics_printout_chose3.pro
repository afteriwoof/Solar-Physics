...
pro front_ell_kinematics_printout_chose3, image, errs, in, da, ang, pos 
	...
	for i=0,sz_xf[0]-1 do begin
		plots, x_front[i], y_front[i], psym=2, symsize=0.5;, color=3
		read, 'Front? y/n', ans
	...
	wset, 0
	plot_map, map, /log, /notitle, /nolabel, /noaxes, /noxticks, /noyticks, /isotropic, pos=pos
	draw_circle, 0, 0, r_sun[2]
	plots, my_xfront, my_yfront, psym=1, symsize=0.5;, color=6
	x_front = my_xfront
	y_front = my_yfront
	...
	plots, p[2], p[3], psym=6, symsize=0.5;, color=6
	...
	if angans eq 'n' then begin
		...
		plots, x_ellipse, y_ellipse
		plots, p[2], p[3], psym=2, symsize=0.5;, color=5
		ang+=1 mod 4
		goto, jump2
	endif	
