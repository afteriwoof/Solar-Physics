...
; ERRORS
; Error in time is just a relationship.
delta_tc2 = fltarr(n_elements(timeplotc2))
delta_tc3 = fltarr(n_elements(timeplotc3))
delta_t = fltarr(n_elements(t))
delta_tc2[0] = 2*(timeplotc2[1]-timeplotc2[0])/3.
delta_tc3[0] = 2*(timeplotc3[1]-timeplotc3[0])/3.
;delta_t[0] = 2*(t[1]-t[0])/3.
delta_tc2[n_elements(timeplotc2)-1] = 2*(timeplotc2[n_elements(timeplotc2)-1]-timeplotc2[n_elements(timeplotc2)-2])/3.
delta_tc3[n_elements(timeplotc3)-1] = 2*(timeplotc3[n_elements(timeplotc3)-1]-timeplotc3[n_elements(timeplotc3)-2])/3.
;delta_t[n_elements(t)-1] = 2*(t[n_elements(t)-1]-t[n_elements(t)-2])/3.
for i=1,n_elements(timeplotc2)-2 do delta_tc2[i] = (timeplotc2[i+1]-timeplotc2[i])/3.
for i=1,n_elements(timeplotc3)-2 do delta_tc3[i] = (timeplotc3[i+1]-timeplotc3[i])/3.
;for i=1,n_elements(t)-2 do delta_t[i] = (t[i+1]-t[i])/3.
