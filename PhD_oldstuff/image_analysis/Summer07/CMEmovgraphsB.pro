...
pro CMEmovgraphsB, pngfls, in, da
	...
	while ( i lt sz_fls[0]) do begin

		print, 'sz_fls[0] ', sz_fls[0]
		print, 'i: ', i		
		pngfl1 = pngfls[i]
		
		if strlen(pngfl1) le 11 then begin
		
			print, 'strlen(pngfl1) le 11'
			ang = 0	
			front = arr[*,*,i]
			...
			if strlen(pngfl1) eq 12 then begin
				print, 'strlen(pngfl1) eq 12'
				front1 = arr[*,*,i]
				if (i+1) lt sz_fls[0] then i+=1
			...
			if case8 eq 1 then eight_fronts, all, front1, front2, front3, front4, front5, front6, front7, $
				front8
			...
			i+=1	
			j += 1
			print, 'i final ', i & print, 'j final ', j
			
		endelse

	endwhile

	save, hcB, f='height_centreB.sav'
	save, haB, f='height_apexB.sav'
	save, angwB, f='angular_widthB.sav'
	save, raB, f='ratioB.sav'
	save, etB, f='ellipse_tiltB.sav'
	save, semimajB, f='semimajorB.sav'
	save, semiminB, f='semiminorB.sav'
	
	wr_movie, 'Behind_ell_movie', mov
