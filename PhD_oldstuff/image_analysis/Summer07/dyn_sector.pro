; Taken from CME_dynamic.pro and includes attempt to chain pixels by their angle info.
...
pro dyn_sector, tangents=tangents, previous=previous, lines=lines
...
restore, '~/PhD/Data_sav_files/diffn.sav'
...
;**************************************************************************************
	...
	; the maximum gradient at each.
		...
		mod_thr = max(im_decomp) * 0.0001
	;	print, 'mod_thr:', mod_thr
		alp_thr = 45.
			...
			;	pmm, im_decomp
				...
				for tempx=0,2 do begin
					for tempy=0,2 do begin
						...
						if moddiff gt mod_thr then modtrue = 1.; & print, 'moddiff:' & print, moddiff
						if alpdiff gt alp_thr then alptrue = 1.; & print, 'alpdiff:' & print, alpdiff
						
					;	print, 'alpdiff:' & print, alpdiff	
					;	read,'go on', ans
						...
						if modtrue eq 1. then begin
							if alptrue eq 1. then begin
								...
								perpdiff1 = abs(perp2-direct1[tempx,tempy])
								perpdiff2 = abs(perp2-direct2[tempx,tempy])
					;			print, 'perpdiff1:' & print, perpdiff1
							;	read, 'on?', ans
								if perpdiff1 gt 10. then chained_edges[xpix,ypix] = 1.	
									;plots, xpix, ypix, psym=5, color=5
								if perpdiff2 gt 10. then chained_edges[xpix,ypix] = 1.
									;plots, xpix, ypix, psym=5, color=5
				...
				set_line_color
				if chained_edges[xpix,ypix] ne 0. then plots, xpix, ypix, psym=3, color=10
