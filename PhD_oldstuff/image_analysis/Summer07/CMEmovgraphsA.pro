...
pro CMEmovgraphsA, pngfls, in, da
	...
	while ( i lt sz_fls[0]-1 ) do begin
		...
		if strlen(pngfl1) le 11 then begin
			...
			ang = 0	
			front = arr[*,*,i]
			front_ell_info, front, in[j], da[*,*,j], height_centre, $
			...
			raA[j] = ratio
			etA[j] = ell_tilt
			semimajA[j] = semimajor
			semiminA[j] = semiminor
			...
			ang=0
			front_full_ell, all, in[j], da[*,*,j], height_centre, $
				height_apex, aw, ratio, ell_tilt, semimajor, semiminor, ang
	...
	save, hcA, f='height_centreA.sav'
	save, haA, f='height_apexA.sav'
	save, angwA, f='angular_widthA.sav'
	save, raA, f='ratioA.sav'
	save, etA, f='ellipse_tiltA.sav'
	save, semimajA, f='semimajorA.sav'
	save, semiminA, f='semiminorA.sav'
	
	wr_movie, 'ahead_ell_movie', mov
