; Code will take only the relevant sector from the estimate of where the CME is in rundiff.

; Code which produces normalised exposure time data array
...
; Want to use the difference technique to give an estimate of the disc which the CME lies in.

; Last edited 14-06-07
...
pro dyn_rundiff_sect, tangents=tangents, previous=previous, lines=lines
...
;for i=0,sz[2]-1 do begin
	...
	; Remove inner disk
;	im = rm_inner(da[*,*,i], in[i], dr_px, thr=2.2)
	
	; Median operator to smooth noise pixels
;	dafn[*,*,i] = fmedian(im, 5, 3)

	; Normalising the data with regard to exposure time
;	dafn[*,*,i] = dafn[*,*,i] / in[i].exptime

;endfor

restore, '~/PhD/Data_sav_files/dafn.sav'

;********
; Creating the background image as in minpix.pro but the median pixel rather than the minimum,

bkgrd = median(dafn, dim=3)
...
; features are highlighted by the running difference.
features = fltarr(sz[0], sz[1], sz[2])
featuresb = features

for i=0,sz[2]-2 do begin
	features[*,*,i] = dafn[*,*,i+1] - dafn[*,*,i]
	featuresb[*,*,i] = bytscl(features[*,*,i], -10, 10)
endfor

szd = size(features, /dim)
newin = in[0:szd[2]-1]
; Creating maps but note the header isn't being edited (the times will be wrong)!
index2map, newin, features, fmaps
index2map, newin, featuresb, fmapsb
szmap = size(fmaps.data, /dim)

;For making the movie: 
;temparr = fltarr(992, 939, szd[2]*3)
...
for i=0,szd[2]-1 do begin
	...
	annulus = ring - mu[0]
	temp = -mu[0]
	mu = moment(annulus[where(annulus ne temp)], sdev=sdev)
	thresh = mu[0] + 0.1*sdev
...
;		print, radius_thresh
		...
		;****************
		; Finding the sector of CME position
		...
		last_r = r_front[sz_r_front]
		
		sector = rm_sector(disc, in[i], dr_px, thr1=first_a, thr2=last_a, mask)
	
		index2map, in[i], sector, discmap
		plot_map, discmap	
	
	;	goto, endjump ;just skipping the wavelet analysis for now.
	...
	; From CME_multiscale.pro
		...
		restore, '~/PhD/Data_sav_files/temp_bkgrd_ave.sav'
;		plot, bkgrd_average
	
		r_thresh = radius_thresh / in[i].cdelt1
		if r_thresh gt sz[0]/2. then r_thresh=500.
	...
	; With this New dynamic threshold, now go through the code again!
		...
		;newim2_decomp = fltarr(sz_decomp[0], sz_decomp[1])
			...
			;********************
	                ...
	                sect = mult * mask
	                index2map, in[i], sect, anglemap
	                plot_map, anglemap
			;*********************
		
			pmm, sect
			print, 'thresh_dyn:', thresh_dyn
		contour, sect, lev=thresh_dyn, /over, path_xy=xy, path_info=info, $
			/path_data_coords	
		plots, xy[0,*], xy[1,*], psym=3
		...
		for l=0,0 do begin
			...
			read, 'xy countour map', ans

			;***********************
			; Calling function ellipsefit.pro to plot ellipse 
			ellipse = ellipsefit(x_dyn, y_dyn)
			aprime = ellipse[0:(size(ellipse,/dim)-2)/2-1]
			bprime = ellipse[(size(ellipse,/dim)-2)/2:(size(ellipse,/dim)-2)-1]
			aprime = (aprime-in[i].crpix1) * in[i].cdelt1
			bprime = (bprime-in[i].crpix2) * in[i].cdelt2
			...
			; Average point of front
			sz_x_front_dyn = size(x_front_dyn, /dim)
			sz_y_front_dyn = size(y_front_dyn, /dim)
			sumx = 0.
			sumy = 0.
			count = 0.
			while (count lt sz_x_front_dyn[0]) do begin
				sumx = sumx + x_front_dyn[count]
				sumy = sumy + y_front_dyn[count]
				count = count + 1
			endwhile

			x_front_bar_dyn = sumx / sz_x_front_dyn[0]
			y_front_bar_dyn = sumy / sz_y_front_dyn[0]
			recpol, x_front_bar_dyn, y_front_bar_dyn, r_bar_dyn, a_bar_dyn, /degrees
			temp = where(a_dyn eq round(a_bar_dyn))
			temp2 = intarr(1)
			temp2[0] = -1
			if temp ne temp2 then begin
				r_a_bar_dyn = max(r_dyn[temp])
				polrec, r_a_bar_dyn, a_bar_dyn, x_apex_dyn, y_apex_dyn, /degrees
				plots, x_apex_dyn, y_apex_dyn, psym=5
			endif else begin
				r_a_bar_dyn = 0.
				x_apex_dyn = 0.
				y_apex_dyn = 0.
		...
		;	temp = where(x_front_dyn2 gt 1023)
		;	if temp ne -1 then x_front_dyn2[temp] = 1023.
		;	temp = where(y_front_dyn2 gt 1023)
		;	if temp ne -1 then y_front_dyn2[temp] = 1023.
		...
		count = 1
		sign = 0
		crossing = where(r_dyn eq round(r_bar_dyn))
		range = [crossing]
		while (size(range, /dim) lt n) do begin
			if sign mod 2 eq 0 then crossings = where(r_dyn eq round(r_bar_dyn)+count)
			if sign mod 2 eq 1 then crossings = where(r_dyn eq round(r_bar_dyn)-count)
			range = [range, [crossings]]
			count = count + 1
			sign = sign + 1
		endwhile

		a_width = max(a_dyn[range]) - min(a_dyn[range])
		polrec, r_bar_dyn, max(a_dyn[range]), x_width1, y_width1, /degrees
		polrec, r_bar_dyn, min(a_dyn[range]), x_width2, y_width2, /degrees
		...
		r_sun = pb0r(anglemap.time, /soho, /arcsec)
		...
		if keyword_set(tangents) then begin
			...
			slope1 = sqrt((y_front_dyn[0] / x_front_dyn[0])^2)
			y_edge1 = (in[i].naxis2 - in[i].crpix2) * in[i].cdelt2
			x_edge1 = y_edge1/slope1
			if y_front_dyn[0] lt 0 then begin
				y_edge1 = -y_edge1
				if x_front_dyn[0] lt 0 then begin
					x_edge1 = -x_edge1
				endif
			endif else begin
				if x_front_dyn[0] lt 0 then begin
			...
			szx_front = size(x_front_dyn, /dim)
			szy_front = size(y_front_dyn, /dim)
		
			slope2 = sqrt((y_front_dyn[szy_front] / x_front_dyn[szx_front])^2)
       		 	y_edge2 = (in[i].naxis2 - in[i].crpix2) * in[i].cdelt2
       		 	x_edge2 = y_edge2/slope2
       		 	if y_front_dyn[szy_front] lt 0 then begin
       		        	y_edge2 = -y_edge2
       		        	if x_front_dyn[szx_front] lt 0 then begin
       		                	x_edge2 = -x_edge2
       		         	endif   
		       	endif else begin
		                if x_front_dyn[szx_front] lt 0 then begin
		...
		;if keyword_set(lines) then begin
			...
			while (count le a_maxd) do begin
				angles[temp] = count
				if(where(a_dyn eq round(count)) eq [-1]) then goto, jump2
				r_angles[temp] = max(r_dyn[where(a_dyn eq round(count))]) 
				jump2:
				r_mid[temp] = r_bar_dyn
				r_front[temp] = r_a_bar_dyn
				temp = temp + 1
				count = count + steps
			endwhile		
