...
pro CMEgraphsAB, fitsflsA, pngflsA, inA, daA, fitsflsB, pngflsB, inB, daB, savA, savB
	...
	while ( i lt sz_pngflsA[0]-1) do begin
		...
		if strlen(fl1A) le 11 then begin
			...
			if strlen(fl1A) eq 12 then begin
				...
				front1B = arrB[*,*,i]
				i += 1
				if strlen(pngflsA[i]) eq 12 then begin
					front2A = arrA[*,*,i]
					front2B = arrB[*,*,i]
					if (i+1) lt sz_pngflsA AND (i+1) lt sz_pngflsB then begin
						i += 1
						res = strpos(pngflsA[i], '_c')
				...
				front1B = arrB[*,*,i]
				i+=1
				if strlen(pngflsA[i]) eq 13 then begin
					front2A = arrA[*,*,i]
					front2B = arrB[*,*,i]
					i+=1
					res = strpos(pngflsA[i], '_c')
