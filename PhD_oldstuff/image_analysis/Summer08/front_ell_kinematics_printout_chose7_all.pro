...
; This uses all the front information read in while only printing out specific steps along the front for illustration.

; Last Edited: 07-07-08

pro front_ell_kinematics_printout_chose7_all, image, errs, in, da, ang, pos, smap
	...
	;plot_map, map, /log, /limb, pos=pos
	;plots, x_map, y_map, psym=1, symsize=0.5
	...
	polrec, r_front, a_front, x_front_plot, y_front_plot, /degrees
	...
	non_zero_front, x_front_plot, y_front_plot, xnf_plot, ynf_plot
	x_front=xnf
	y_front=ynf
	;plots, x_front, y_front, psym=1, symsize=0.5;, color=2;yellow
	...
	for i=0,sz_xf[0]-1 do begin
		plots, x_front[i], y_front[i], psym=2, symsize=0.2, color=255
		read, 'Front? y/n', ans
	...
	;plot_map, map, /log, /notitle, /nolabel, /noaxes, /noxticks, /noyticks, pos=pos, /isotropic
	;plot_map, smap, /limb, /log, /notit, /nolabel, /noaxes, pos=pos, /isotropic
	;draw_circle, 0, 0, r_sun[2]
	;plots, my_xfront, my_yfront, psym=1, color=255
	plots, xnf_plot, ynf_plot, psym=1, color=255
	x_front = my_xfront
	y_front = my_yfront
	...
	plots, x_ellipse, y_ellipse, color=255
	;plots, p[2], p[3], psym=6, symsize=0.2, color=255
	...
	if angans eq 'n' then begin
		...
		plots, p[2], p[3], psym=2, symsize=0.2;, color=5
		ang+=1 mod 4
		goto, jump2
	endif	
	...
	if true eq 0 then begin	
		...
		plots, [0,min_cone_x], [0,min_cone_y], color=255
		plots, [0,max_cone_x], [0,max_cone_y], color=255
