...
pro plot_slices_soho_testing, slice_begin, slice_end, bogus_slices, ledge, angle_x, angle_z, mov, nolabels=nolabels, zoom=zoom, no_ell=no_ell, plot_cone=plot_cone, pauses=pauses, no_proj=no_proj, save_front=save_front, over=over, room_all=room_all
...
for k=slice_begin,slice_end do begin
	if where(bogus eq k) eq [-1] then begin
		...
		if ~keyword_set(no_proj) then begin
			...
			for i=0,3 do plots, x[i], y[i], z[i], color=i+1, psym=2, /t3d
		endif
		...
		; project the quadrilateral vertices onto SOHO plane-of-sky (x=0 plane)
		openw, lun, /get_lun, 'proj_soho_quad.txt'
		free_lun, lun

		project_soho_quad, lun, x, y, z, x3, y3, z3
