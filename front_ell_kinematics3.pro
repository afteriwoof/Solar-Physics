...
; Code which doesn't ignore any of the front edge points automatically!

; Last Edited: 11-02-08

pro front_ell_kinematics3, image, errs, in, da, ang, ell_info, r_err, x_ellipse, y_ellipse, p
...
;	r = round(r)
	...
	a_front_no = fltarr(a_max - a_min +1)
	r_front_no = fltarr(a_max - a_min +1)
	temp = 0.
	count = a_min
	stepsize = 1.
	while (count le a_max) do begin
		a_front_no[temp] = count
		if (where(a eq count) eq [-1]) then goto, jump1
		r_front_no[temp] = max(r[where(a eq count)])
		jump1:
		temp = temp + 1
		count = count + stepsize
	endwhile

	polrec, r_front_no, a_front_no, x_front_no, y_front_no, /degrees
	a_front = a
	r_front = r
	x_front = x_map
	y_front = y_map
	...
	my_xfront = fltarr(sz_xf[1])
	my_yfront = fltarr(sz_yf[1])
	count = 0
	read, 'Specify front? y/n ', ans
	if ans eq 'n' then begin
		for i=0,sz_xf[1]-1 do begin
