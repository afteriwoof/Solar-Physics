...
pro read_daily_stacks, stack_fls, in_dir, new_det_info, new_pa_total, count_days, single_file=single_file, debug=debug, show=show, no_save=no_save
...
for i=0,n_elements(stack_fls)-loop_end do begin
	...
	pa_total = rm_prev_date(stack_fls[i], in_dir, out_check, debug=debug)
	if keyword_set(debug) then begin
	...
	if inds ne [-1] then begin
		...
		if keyword_set(single_file) then begin
			...
			if strlen(test) ne 0 then begin
				...
				temp = rm_prev_date(next_stack_fl, temp_in_dir, temp_out_check, debug=debug)
				count = 1
			endif else begin
				goto, jump1
			endelse
