...
mreadfits_corimp, fls[count-1], in0, da0 & $
mreadfits_corimp, fls[count], in, da & $

sz = size(da,/dim) & $

mean0 = (moment(da0[where(da0 ne 0)]))[0] & $
mean1 = (moment(da[where(da ne 0)]))[0] & $
diff = da - da0*(mean1/mean0) & $

mu = moment(diff, sdev=sdev) & $
contour, diff, lev=mu[0]+sdev, path_info=info, path_xy=xy, /path_data_coords & $

diffwin = intarr(sz[0],sz[1]) & $
no_contours = 4 & $
for c=0,no_contours-1 do begin & $
cx = xy[0,info[c].offset:(info[c].offset+info[c].n-1)] & $
cy = xy[1,info[c].offset:(info[c].offset+info[c].n-1)] & $
cind = polyfillv(cx,cy,sz[0],sz[1]) & $
diffwin[cind] = 1 & $
endfor & $
se = intarr(12,12) & $
se[*,*] = 1 & $
diffwin = dilate(diffwin, se) & $
...
multmask = multmods * cme_mask_thr * diffwin & $
