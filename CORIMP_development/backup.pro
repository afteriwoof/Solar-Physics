...
pro run_algorithms2_edges_tvscl, in, da, count, multmods, cme_mask_thr_unique, edges, contour_count, out_dir, af_out, heights, fail, show=show, final_show=final_show, pauses=pauses, remove_streamers=remove_streamers, fit_ellipse=fit_ellipse
...
print, 'Eroding the multmask_binary by 9x9'
...
print, 'Currently using (edges[*,*,1]+edges[*,*,2]) from wtmm, corresponding to array entry scales 4 and 5 of the canny_atrous'
if keyword_set(show) then begin
...
yf = y+ave(xy[1,*]) & $
set_line_color
if keyword_set(show) then plots, xf, yf, psym=1, color=3, /device
if keyword_set(pauses) then pause
...
if fail eq 1 then begin
        ...
        if contour_count eq 0 then if keyword_set(show) or keyword_set(final_show) then begin
		loadct,0
		tvscl, da 
	        legend, 'Detection area too small', charsize=2,/left
		if keyword_set(pauses) then pause
	endif
endif
...
if fail ne 1 then begin
	xc = in.xcen
	yc = in.ycen
	print, 'save, da, flank1x, flank1y, flank2x,flank2y,res,xf_out,yf_out,xc,yc'
