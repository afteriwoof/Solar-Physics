; Edited	2012-03-19	
; Last edited:	2012-05-04
;		2012-05-24	to change the det_stack input for cases where there's more than one stack (ie spanning days).
...
pro find_pa_heights_all_redo, fls_in, pa_total, det_info, det_fls, stack_fls, out_dir, plot_prof=plot_prof, loud=loud, pauses=pauses
...
;looping over the separate CME detections
	...
	help, det_info, hdrs
	print, det_info[2,i]
	print, hdrs[det_info[2,i]].date_obs
	file_out = out_dir+'/'+anytim(hdrs[det_info[2,i]].date_obs,/stime)+'.txt'
	print, 'file_out ', file_out
	openw, lun, /get_lun, file_out, error=err, width=500
	printf, lun, '# HEIGHT, DATE & TIME, ANGLE'
	printf, lun, '#'
	free_lun, lun
	...
	for k_count=det_info[0,i]+20,det_info[1,i] do begin
	;looping over the relevant angles in the detection
		...
		cme_hdrs = hdrs[det_info[2,i]:det_info[3,i]]
		...
		;looping over the detections at this angle
			...
			help, plots_list, plots_list_j, plots_list_j[0], offset
			...
			if cnt2 ne 0 then begin
				...
				; Save out the height info to a text file
	                        ...
	                        printf, lun, hf, ' ', in.date_obs, ' ', (k_count mod 360)
       	                	free_lun, lun
			endif
