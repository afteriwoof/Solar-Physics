pro bootstrap_quad_171, date, grt_dist_171, time171, h_err, dist_arr_171, f_171_quad, conf_range_r, $
		conf_range_v, conf_range_a, plot_on=plot_on, print_on=print_on
...
; Known equation we want to recover (100 data points)
	...
	data_fit_171_quadratic, grt_dist_171, time171, h_err, dist_arr_171, f_171_quad, perror

	y = f_171_quad[0] + f_171_quad[1]*x + (1./2.)*f_171_quad[2]*x^2.
...
; Add Gaussian noise mean=0, sigma =1 of differing amplitudes to m and c params
	...
	rnoise = RANDOMN( Seed1, 100.0d , /DOUBLE, /NORMAL )*perror[0] + f_171_quad[0]
	vnoise = RANDOMN( Seed2, 100.0d , /DOUBLE, /NORMAL )*perror[1] + f_171_quad[1]
	anoise = RANDOMN( Seed3, 100.0d , /DOUBLE, /NORMAL )*perror[2] + f_171_quad[2]
...
; Plot histograms and over plot Gaussian with mean and sigma just calculated
		...
		plot_hist2, res[0,*], xr,hr, bin=0.01, /xs, xr=[p1[0] - 3*s1, p1[0] + 3*s1], charsize=2.0,/density, $
				tit='R0', color = 0, /nodisp
				
	if (keyword_set(plot_on)) then begin
				
		plots, [f_171_quad[0], f_171_quad[0]], [0, max(hr)], color=3
		plots, [p1[0], p1[0]], [0, max(hr)], color=5
;		verline, 10.0d, color=3
;		verline, p1[0], color=5, lines=2
		legend,['Actual value: '+num2str(f_171_quad[0]) + ', error: '+num2str(perror[0]), $
...
;		xyouts, 7.1, 90, 'Mean: '+string(p1[0])+', Sigma: '+string(s1), chars=1.2, color = 0
	...
	if (keyword_set(plot_on)) then begin
		...
		plots, [f_171_quad[1], f_171_quad[1]], [0, max(hv)], color=3
		plots, [p2[0], p2[0]], [0, max(hv)], color=5
;		verline, 0.5d, color=3
;		verline, p2[0], color=5, lines=2
		legend,['Actual value: '+num2str(f_171_quad[1]) + ', error: '+num2str(perror[1]), $
...
;		xyouts, 0.405, 280, 'Mean: '+string(p2[0])+', Sigma: '+string(s2), chars=1.2, color = 0
		...
		plot_hist2, res[2,*], xa, ha, bin=0.0000001, /xs, xr=[p3[0] - 3*s3, p3[0] + 3*s3], /nodisp, $
				charsize=2.0,/density,tit='V0', color = 0

	if (keyword_set(plot_on)) then begin

		plots, [f_171_quad[2], f_171_quad[2]], [0, max(ha)], color=3
		plots, [p3[0], p3[0]], [0, max(ha)], color=5
;		verline, 0.5d, color=3
;		verline, p2[0], color=5, lines=2
		legend,['Actual value: '+num2str(f_171_quad[2]) + ', error: '+num2str(perror[2]), $
...
; Print out some comparisons 
		...
		Print, 'Original line def y = (!U1!N/!D2!N)' + num2str(f_171_quad[2]) + 'x!U2!N + ' + num2str(f_171_quad[1]) + $
				'*x + ' + num2str(f_171_quad[0])
		print, 'R0 = ' + num2str(f_171_quad[0]) + ' Mm, v0 = ' + num2str(1000*f_171_quad[1]) + ' km/s, a0 = ' + $
				 num2str(1e6*f_171_quad[2]) + ' m/s/s
