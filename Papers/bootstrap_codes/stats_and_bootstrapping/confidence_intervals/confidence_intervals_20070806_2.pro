...
pro confidence_intervals_20070806_2
...
; Restore confidence interval data - R0 195 Lin
	...
	restore, 'CI_' + num2str(date) + '_' + num2str(event) + '_0.sav'
	...
	for i = 1, j do begin
		restore, 'CI_' + num2str(date) + '_' + num2str(event) + '_' + num2str(i) + '.sav'

		oplot, [conf_range_r_195_lin[0], conf_range_r_195_lin[1]], [i, i], color = 0
	
	endfor

	restore, 'CI_' + num2str(date) + '_' + num2str(event) + '_average.sav'
