; Routine for the resampling residual bootstrapping method. 195A quadratic fit.

pro resample_residual_195_quad, date, grt_dist_195, time195, h_err, dist_arr_195, $
					f_195_quad, perror, p1, s1, p2, s2, p3, s3, conf_range_r, conf_range_v, $
					conf_range_a
...
; original fit to data
	...
	yfit = f_195_quad[0] + f_195_quad[1]*tim195 + (1./2.)*f_195_quad[2]*tim195^2.
...
; Plot original data with fitted line
	...
	plot, tim195, grt_dist_195, Title ='Original data and fit - 195 Quadratic', xtit='X', ytit='Y', charsize=2.0, $
		background = 1, color = 0, xr = [-50, max(tim195) + 50], /xs, yr = [0, max(grt_dist_195) + 50], /ys, $
		psym = 2
	oplot, x_arr, y_line, color=3
	legend,['Data', 'Fit: r = '+ num2str(f_195_quad[0]) + ' + ' + num2str(f_195_quad[1]) + '*x + !U1!N/!D2!N ' + $
		num2str(f_195_quad[2]) + '*x!U2!N'], colors=[0, 3], chars=1.2, /right, /bottom, /clear, textcolor = [0, 3]
