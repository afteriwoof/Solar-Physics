...
fls_cor2a = file_search('~/Postdoc/Automation/candidates/secchi_cor2_a/2011/01/13/sep*cme.fits.gz')
fls_cor2b = file_search('~/Postdoc/Automation/candidates/secchi_cor2_b/2011/01/13/*cme.fits.gz')
fls_c3 = file_search('~/Postdoc/Automation/candidates/201101/13/sep*C3.cme.fits.gz')
...
plots_c3 = file_search('~/Postdoc/Automation/Catalogue/20110112-14/plots_20110113*C3.sav')

time_cor2a = yyyymmdd2cal(strmid(file_basename(fls_cor2a),26,12))
time_cor2b = yyyymmdd2cal(strmid(file_basename(fls_cor2b),1,12))
time_c3 = yyyymmdd2cal(strmid(file_basename(fls_c3),25,12))
...
;for ib=0,1 do begin & $
	...
	device, xsize=devxs, ysize=devys, bits=8, language=2, /portrait, /inch, /color, filename='animation3_'+int2str(ib)+'.eps', /encapsul
