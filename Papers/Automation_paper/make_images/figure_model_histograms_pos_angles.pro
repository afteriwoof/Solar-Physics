; Code to plot postscript image of multicme_model velocity histograms with colorbar corresponding to pos_angles

; Created:	10-09-11	;from figure_model_histograms.pro 


pro figure_model_histograms_pos_angles

toggle, /portrait, /color, f='figure_model_histograms_pos_angles.ps'
!p.charsize=2
!p.charthick=3
!p.thick=1
...
utplot, t, (heights_km/1000.), utbase, psym=1, ytit='Height (Mm)', pos=[0.1,0.81,0.5,1],/nodata, $
...
pos_ind = where(pos_angles gt 180, cnt)
if cnt gt 0 then pos_angles[pos_ind] -= 360
set_line_color
for i=min(pos_angles),max(pos_angles) do begin
	...
	if cnt gt 0 then outplot, new_t[pos_ind], new_heights[pos_ind]/1000., $
...
utplot, t, (heights_km/1000.), utbase, psym=1, ytit='Height (Mm)', pos=[0.1,0.61,0.5,0.8],/nodata, $
        xr=[tx1,tx2], /xs, yr=[0,1.3e4], /ys, color=3, /nolabel, xtickname=[' ',' ',' ',' ',' ',' ',' ']
outplot, new_t, new_heights/1000., psym=1, color=0
