...
devxs = 2348.
devys = 2886.
device, xsize=devxs/200., ysize=devys/200., bits=8, language=2, /portrait, /cm, /color, filename='figure_events_secchi.ps'
pos_x = [624,724,1348,1498,2248]
pos_y = [2262,2072,1448,1378,754,624]
...
fl=file_Search('../../candidates/secchi_cor2_a/2011/01/13/separated_secchi_20110113_201101131139__C2.orig.fits.gz')
...
fls=file_search('~/Postdoc/Automation/candidates/201101/13/sep*cme*')
fls = sort_fls(fls)
...
pos = [ (pos_x[1]+100)/devxs, pos_y[0]/devys, (pos_x[2]+100)/devxs, 1 ]

xls = (anytim(time[0]))-utbasedata + 6500
xrs = (anytim(time[n_elements(time)-1])) - utbasedata + 3500
...
utplot, t, (heights_km/1000.), utbasedata, psym=1, ytit='Height (Mm x10!U4!N)', pos=pos, $
	...
	ytickname=['0','0.5','1.0','1.5','2.0'];, tit='HEIGHT-TIME PROFILE'
...
horline, 3.*700., linestyle=1
horline, 11.*700., linestyle=1

;colorbar

pos = [ (pos_x[4]-760)/devxs, pos_y[0]/devys, (pos_x[4]-730)/devxs, 1 ]
...
pos = [ (pos_x[1]+100)/devxs, pos_y[2]/devys, (pos_x[2]+100)/devxs, pos_y[1]/devys ]

xls = (anytim(time[0]))-utbasedata + 5418
xrs = (anytim(time[n_elements(time)-1])) - utbasedata + 2000

utplot, t, (heights_km/1000.), utbasedata, psym=1, ytit='Height (Mm x10!U4!N)', pos=pos, $
	xr=[t[0]-xls,t[*]+xrs], /xs, /nodata, charsize=my_charsize, charthick=my_charthick, $
	ytickname=['0','0.5','1.0','1.5','2.0']
...
horline, 4.5*700., linestyle=1
horline, 11.*700., linestyle=1

;colorbar

pos = [ (pos_x[4]-760)/devxs, pos_y[2]/devys, (pos_x[4]-730)/devxs, pos_y[1]/devys ]
