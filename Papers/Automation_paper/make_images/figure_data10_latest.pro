...
;Created:	20120314	from figure_data10_orig.pro with the latest processed dynamic files off gail.

pro figure_data10_latest
...
; 		x:	180-624-120-624-200		180-804-924-1548-1748
;		y:	180-624-30-624-30-624-30-624-30-624	180-804-834-1458-1488-2112-2142-2766-2796-3420
devxs = 1748.
devys = 3450.
device, xsize=devxs/175., ysize=devys/175., bits=8, language=2, /portrait, /cm, /color, filename='figure_data10_latest.eps', /encapsul
...
fls=file_Search('~/Postdoc/Automation/Test/201003_gail/12/dyn*')
fls=sort_fls(fls)
mreadfits_corimp, fls[22], inc2, dac2

if inc2.xcen lt 0 then inc2.xcen=inc2.crpix1
if inc2.ycen lt 0 then inc2.ycen=inc2.crpix2
...
xyouts, pos[0]+posaddx, pos[3]-posaddy, '(a)', /norm, charsize=my_charsize-charfac, charthick=my_charthick

; C3 DATA

;Fix which is the correct file number to be calling - not 51?!
mreadfits_corimp, fls[52], inc3, dac3
if inc3.xcen lt 0 then inc3.xcen=inc3.crpix1
if inc3.ycen lt 0 then inc3.ycen=inc3.crpix2
...
pos = [ 924/devxs, 2796/devys, 1548/devxs, 3420/devys ]
...
dac3 = hist_equal(dac3,percent=0.1)
dac3[nfov] = max(dac3)
...
xyouts, pos[0]+posaddx, pos[3]-posaddy, '(b)', /norm, charsize=my_charsize-charfac, charthick=my_charthick

;Add comet and planet labels
xyouts, 0.9, 0.85, 'comet', /norm, charsize=my_charsize, charthick=my_charthick
xyouts, 0.9, 0.87, 'Mercury', /norm, charsize=my_charsize, charthick=my_charthick
set_line_color
arrow, 0.895, 0.852, 0.625, 0.87, /normalized, color=3, thick=my_charthick, hthick=my_charthick
arrow, 0.895, 0.873, 0.784, 0.873, /normalized, color=3, thick=my_charthick, hthick=my_charthick
loadct, 0
...
mods = hist_equal(mods,percent=0.1)
mods[nfov] = max(mods)
...
xyouts, pos[0]+posaddx, pos[3]-posaddy, '(c)', /norm, charsize=my_charsize-charfac, charthick=my_charthick
...
alps = hist_equal(alps,percent=0.1)
alps[nfov] = max(alps)
...
xyouts, pos[0]+posaddx, pos[3]-posaddy, '(e)', /norm, charsize=my_charsize-charfac, charthick=my_charthick
...
pos = [ 924/devxs, 2142/devys, 1548/devxs, 2766/devys ]
...
xyouts, pos[0]+posaddx, pos[3]-posaddy, '(d)', /norm, charsize=my_charsize-charfac, charthick=my_charthick
...
pos = [ 924/devxs, 1488/devys, 1548/devxs, 2112/devys ]
...
xyouts, pos[0]+posaddx, pos[3]-posaddy, '(f)', /norm, charsize=my_charsize-charfac, charthick=my_charthick

;colorbar
loc = [ 1565/devxs, 1488/devys, 1600/devxs, 2112/devys ]
...
fls=file_Search('~/Postdoc/Automation/Test/201003_gail/12/dyn*')
fls = sort_fls(fls)
mreadfits_corimp, fls[22], inc2
mreadfits_corimp, fls[52], inc3

restore, 'CME_mask_20100312_050604_C2.sav'
cme_maskc2 = cme_mask
restore, 'CME_mask_20100312_114203_C3.sav'
cme_maskc3 = cme_mask
delvarx, cme_mask
...
;cme_maskc2 = hist_equal(cme_maskc2,percent=0.1)
...
xyouts, pos[0]+posaddx, pos[3]-posaddy, '(g)', /norm, charsize=my_charsize-charfac, charthick=my_charthick
...
pos = [ 924/devxs, 834/devys, 1548/devxs, 1458/devys ]

fov = where(ht ge 6 and ht lt 20, comp=nfov)
cme_maskc3 = (float(hist_equal(cme_maskc3>0,per=0.1,omin=mn,omax=mx,minv=0,maxv=maxv))*254/255.)
cme_maskc3[nfov] = upper_val
restore,'~/idl_codes/c3pylonmask.sav'
;c3pylonmask = c3pylonmask[146:877,146:877]
pylon_inds = where(c3pylonmask eq 0)
cme_maskc3[pylon_inds] = upper_val
;cme_maskc3 = hist_equal(cme_maskc3,percent=0.1)
...
xyouts, pos[0]+posaddx, pos[3]-posaddy, '(h)', /norm, charsize=my_charsize-charfac, charthick=my_charthick

;colorbar  
loc = [ 1565/devxs, 834/devys, 1600/devxs, 1458/devys ]
...
fls = file_Search('~/Postdoc/Automation/Test/201003/12/*orig*')
fls = sort_fls(fls)
mreadfits_corimp, fls[21], inc2
mreadfits_corimp, fls[51], inc3

; restores in, da, res, xf_out, yf_out
restore, 'dets_20100312_050604_C2.sav'

cme_detectionc2 = readfits_corimp(fls[21],inc2)
cme_detectionc2 = congrid(cme_detectionc2, 732, 732)
...
cme_detectionc2 = hist_equal(cme_detectionc2,percent=0.1)
...
xyouts, pos[0]+posaddx, pos[3]-posaddy, '(i)', /norm, charsize=my_charsize-charfac, charthick=my_charthick


; C3 DETECTIONS

fls_dyn=file_Search('~/Postdoc/Automation/Test/201003_gail/12/dyn*')
mreadfits_corimp, fls_dyn[52], inc3

if inc3.xcen lt 0 then inc3.xcen=inc3.crpix1
if inc3.ycen lt 0 then inc3.ycen=inc3.crpix2
restore, 'dets_20100312_114203_C3.sav'
res = dets.edges
xf_out = dets.front[0,*]
yf_out = dets.front[1,*]
...
cme_detectionc3 = readfits_corimp(fls[51],inc3)
cme_detectionc3 = cme_detectionc3[146:877, 146:877]
inc3.naxis1 = 732
inc3.naxis2 = 732
inc3.crpix1 = 366
inc3.crpix2 = 366
inc3.xcen=inc3.crpix1
inc3.ycen=inc3.crpix2

get_ht_pa_2d_corimp,inc3.naxis1,inc3.naxis2,inc3.crpix1,inc3.crpix2,pix_size=inc3.pix_size/inc3.rsun,x,y,ht,pa

pos = [ 924/devxs, 180/devys, 1548/devxs, 804/devys ]
...
cme_detectionc3 = hist_equal(cme_detectionc3,percent=0.1)
...
set_line_color
plots, res[0,*], res[1,*], psym=8, color=2, symsize=0.04
plots, xsun, ysun, thick=2

xyouts, pos[0]+posaddx, pos[3]-posaddy, '(j)', /norm, charsize=my_charsize-charfac, charthick=my_charthick
