...
pro figure_peak_accel, model_file, in, heights, image_no, pos_angles, new_heights, new_image_no, new_pos_angles, gather_t_nows, gather_h_nows, gather_v_nows, gather_a_nows, tog=tog, plot_quartiles=plot_quartiles, remove_outliers=remove_outliers
...
restore, '~/Postdoc/Automation/Catalogue/cme_model_run_peakaccel_file_increment1_20110920a/nondynamic/all_h0.sav'

pos_angles += 180

model_file = '~/Postdoc/Automation/Catalogue/cme_model_run_peakaccel_file_increment1_20110920a/nondynamic/peakaccel_file_increment1_20110920a.dat'
...
if keyword_set(tog) then begin
	toggle, /portrait, /color, f='figure_peak_accel.eps'
	!p.background=1
	!p.charsize=3.0
	!p.charthick=5 ;axes
	!p.thick=5 ;psym
...
if keyword_set(remove_outliers) then begin
	...
	utplot, t, (heights_km/6.955e5), utbasedata, psym=1, ytit='Height (R!D!9n!N!X)', pos=[0.15,0.70,0.95,0.98], $
		xr=[t[0]-xls,t[*]+xrs], /xs, /nolabel , xtickname=[' ',' ',' ',' ',' ',' ',' '], color=3
	outplot, new_t, new_heights/6.955e5, psym=1, color=char_color
	outplot, t_model, (r_model/60000.), anytim('18-jan-2005T02:00'), psym=-3
endif else begin
	utplot, t, (heights_km/6.955e5), utbasedata, ytit='Height (R!D!9n!N!X)',  pos=[0.15,0.60,0.95,0.88], $
...
for i=min(pos_angles),max(pos_angles) do begin
        ...
        if cnt gt 0 then begin
                outplot, new_t[pos_ind], new_heights[pos_ind]/6.955e5,$
...
;outplot, t_model, (r_model_temp/60000.), anytim('18-jan-2005T02:00')
;;;;;;;;;;;;
; plot line to mark occulter edges/crossover 5.75-6 Rsun
horline, 2.25, linestyle=1
horline, 5.75, linestyle=1
horline, 6, linestyle=2
horline, 16, linestyle=2

xyouts, 12000, 2.6, 'C2', charsize=1.
xyouts, 12000, 6.3, 'C3', charsize=1.
