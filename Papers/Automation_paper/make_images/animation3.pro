...
fls_c3 = file_search('~/Postdoc/Automation/candidates/20110113/original*c3*fits.gz')

cnta = n_elements(fls_cor2a)
cntb = n_elements(fls_cor2b)
cntlasco = n_elements(fls_c3)

dets_cor2a = file_Search('~/Postdoc/Automation/candidates/secchi_files/a/detections_dublin/dets_*sav')
dets_cor2b = file_Search('~/Postdoc/Automation/candidates/secchi_files/b/detections_dublin/dets_*sav')
dets_c3 = file_Search('~/Postdoc/Automation/candidates/20110113/detections_dublin/dets_*C3.sav')

dets_cor2a_times = yyyymmdd2cal(strmid(file_basename(dets_cor2a),5,8) + strmid(file_basename(dets_cor2a),14,6))
dets_cor2b_times = yyyymmdd2cal(strmid(file_basename(dets_cor2b),5,8) + strmid(file_basename(dets_cor2b),14,6))
dets_c3_times = yyyymmdd2cal(strmid(file_basename(dets_c3),5,8) + strmid(file_basename(dets_c3),14,6))

time_cor2a = strmid(file_basename(fls_cor2a), 26, 12)
time_cor2a = yyyymmdd2cal(time_cor2a)
...
;for ib=0,1 do begin & $
	...
	if hdrlasco.xcen le 0 then hdrlasco.xcen=hdrlasco.crpix1
	if hdrlasco.ycen le 0 then hdrlasco.ycen=hdrlasco.crpix2
	...
	if dets_cor2b_times[ib+count_detsb] eq time_cor2b[ib] then begin 
		restore, dets_cor2b[ib+count_detsb]
		res = dets.edges
		xf_out = dets.front[0,*]
		yf_out = dets.front[1,*]
		set_line_color
	...
	if dets_c3_times[indlasco+count_detsc3] eq time_c3[indlasco] then begin
		restore, dets_c3[indlasco+count_detsc3]
		res = dets.edges
		xf_out = dets.front[0,*]
		yf_out = dets.front[1,*]
		set_line_color
	...
	print, 'inda, count_detsa, dets_cor2a_times[inda+count_detsa], time_cor2a[inda], dets_cor2a[inda+count_detsa]', inda, count_detsa, dets_cor2a_times[inda+count_detsa], time_cor2a[inda], dets_cor2a[inda+count_detsa]
	if dets_cor2a_times[inda+count_detsa] eq time_cor2a[inda] then begin
		restore, dets_cor2a[inda+count_detsa]
		res = dets.edges
        	xf_out = dets.front[0,*]
        	yf_out = dets.front[1,*]
		set_line_color
