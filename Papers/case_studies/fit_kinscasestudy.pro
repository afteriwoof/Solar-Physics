; Created:      2012-10-05      to fit a model curve to the CME height-time points.

; Last Edited:	2012-11-28	to output the four eps images for case_studies/ folder.
;		2013-05-02	to ensure the resampling was added to the original fit.

pro fit_kinscasestudy, restore_fl, lagrangian=lagrangian, quadratic=quadratic, cubic=cubic, powerlaw=powerlaw, exponential=exponential, sav_gol=sav_gol, residuals=residuals, fuzzy_error=fuzzy_error, bootstrap=bootstrap, tog=tog, occulters=occulters
...
if keyword_set(tog) then begin
	...
	!y.thick=4
	set_plot, 'ps'
	device, /encapsul, bits=8, language=2, /portrait, /color, filename='fit_kinscasestudy_1.eps', $
		xs=15, ys=20
	print, 'set_plot, ps'
	print, 'Saving fit_kinscasestudy_1.eps'
endif else begin
	!p.charsize=2
	!p.charthick=1
	!p.thick=1
...
if keyword_set(lagrangian) then begin
	...
	;h_fit = h_noisy
	plot, tt, h_noisy/height_factor, psym=1, pos=pos_h, xtickname=[' ',' ',' ',' '], yr=yr_h, ystyle=8
...
if keyword_set(residuals) OR keyword_set(bootstrap) then begin
	...
	if keyword_set(tog) then begin
		!p.charsize=1
	        device, /encapsul, bits=8, language=2, /portrait, /color, filename='fit_kinscasestudy_2.eps', $
			xs=15, ys=8
        	print, 'Saving fit_kinscasestudy_2.eps'
	endif else begin
...
if keyword_set(fuzzy_error) OR keyword_set(bootstrap) then begin
	if keyword_set(tog) then begin
		!p.charsize=2
	        device, /encapsul, bits=8, language=2, /portrait, /color, filename='fit_kinscasestudy_3.eps', $
			xs=15, ys=20
	        print, 'Saving fit_kinscasestudy_3.eps'
	endif else begin
	...
	plot, t, h_fullmodel/height_factor, psym=-3, pos=pos_h, xtickname=[' ',' ',' ',' '], $
		yr=yr_h, ystyle=8, ytit='Height (Mm)', tit='Bootstrap : '+title;, /ylog
	axis, yaxis=1, yrange=yr_h/695.5, /ys, ytit=symbol_corimp('rs')
	...
	if keyword_set(quadratic) then begin
		if keyword_set(tog) then begin
        	        !p.charsize=2
        	        device, /encapsul, bits=8, language=2, /portrait, /color, filename='fit_kinscasestudy_4.eps', $
        	                xs=25, ys=10
        	        print, 'Saving fit_kinscasestudy_4.eps'
        	endif else begin
