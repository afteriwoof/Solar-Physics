...
\titlerunning{Investigating and improving methods for determining the kinematics of solar eruptive events.}
...
In Figure~\ref{noise_test_image} the combined effects outlined in Sections~\ref{subsect:noise} and \ref{subsect:cadence} are simulated for the model coronal wave case. The figure shows the result of plotting derived accelerations against all variations of noise from $0$\,--\,$40\%$ at all variations of cadence from $1$\,--$720\,s$. Essentially the plots of the bottom panels of Figures~\ref{noise_hist_weight} and \ref{cad_hist_weight} represent slices through the corresponding locations of Figure~\ref{noise_test_image}. Thus demonstrating that the reduction of both noise and cadence together is required to reduce the solution space (increase the accuracy) of the derived kinematics.
