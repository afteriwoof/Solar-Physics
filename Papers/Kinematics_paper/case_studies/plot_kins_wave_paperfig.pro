; Created	2013-01-18	from plot_kins_quartiles_paperfig.pro to just do the specifics of the paper figure.
...
;********************
help, heights_in, new_heights
pmm, heights_in, new_heights
new_heights = heights_in
...
if keyword_set(saves) then begin
	...
	save, new_heights, time, t, heights_in, utbasedata, new_t, f='new_heights_info.sav'
endif
...
utplot, t, (new_heights*height_fac), utbasedata, psym=1, ytit='Distance (Mm)', pos=[0.15,0.63,0.95,0.91],yr=[0,h_max_range],/ys, $
...
for i=min(pos_angles),max(pos_angles) do begin
	...
	if cnt gt 1 then begin
		outplot, new_t[pos_ind], new_heights[pos_ind]*height_fac,$
		...
		if keyword_set(linear) AND (n_elements(t_now) ge 2) then begin
			any_fit = 1
                        ...
                        if ~keyword_set(no_plots) then outplot, t_now, h_fit*height_fac, psym=-3, thick=1
		...
		if keyword_set(quadratic) AND (n_elements(t_now) ge 3) then begin
			...
			if flag_init eq 1 then begin
                                ...
                                flag_init = 1
                        endelse
			print, 'kins_counter: ', kins_counter
			print, 'kins_color: ', kins_color
			goto, jump4
		endif
		; SAVITZKY-GOLAY
		if keyword_set(sav_gol) AND (n_elements(t_now) ge 7) then begin
			...
			if ~keyword_set(no_plots) then outplot, t_now, h_fit*height_fac, psym=-3, thick=0.5
...
help, v_all, a_all, t_all
; Plot the velocity corresponding to the heights at each position angle
utplot, t_all[0:kins_counter[0]-1], v_all[0:kins_counter[0]-1]*vel_fac, utbasedata, psym=1, ytit='Velocity (km s!U-1!N)', $
	xr=[xls,xrs], /xs, yr=[vel_min_range,vel_max_range], /ys, pos=[.15,0.34,0.95,0.62], /nolabel, $
	xtickname=[' ',' ',' ',' ',' ',' ',' '], xtit='', /nodata;, color=kins_color[0]
print, 'plotting 0 up to ', kins_counter[0]-1, ' as color ', kins_color[0]
outplot, t_all[0:kins_counter[0]-1], v_all[0:kins_counter[0]-1]*vel_fac, psym=1, color=kins_color[0]
if size(kins_counter,/dim) ne 0 then begin
	...
	for i=1,n_elements(kins_counter)-2 do begin
		if (last_t) lt n_elements(v_all) then begin
			...
			outplot, t_all[first_t:last_t], v_all[first_t:last_t]*vel_fac,psym=1,$
				color=kins_color[i]
			first_t = last_t + 1
	...
	outplot, t_all[first_t:n_elements(t_all)-1], v_all[first_t:n_elements(t_all)-1]*vel_fac, psym=1, color=kins_color[n_elements(kins_color)-1]
endif
loop_exit1:
; PLOT INTERQUARTILES
...
if keyword_set(plot_quartiles) then begin
	outplot, t_bins, v_meds*vel_fac, utbasedata, psym=-3, thick=thick_t, color=c, line=0
	outplot, t_bins, v_q1s*vel_fac, utbasedata, psym=-3, thick=thick_t, color=c, line=2
	outplot, t_bins, v_q3s*vel_fac, utbasedata, psym=-3, thick=thick_t, color=c, line=2
	outplot, t_bins, lowerfence*vel_fac, utbasedata, psym=-3, thick=thick_t, color=c, line=2
	outplot, t_bins, upperfence*vel_fac, utbasedata, psym=-3, thick=thick_t, color=c, line=2
	...
	if keyword_set(plot_median) then begin
		outplot, t_bins, v_meds*vel_fac, utbasedata, psym=-3, thick=thick_t, color=c, line=0
		;horline, median(v_all)*vel_fac, line=4
		;legend, 'Median vel: '+int2str(median(v_all)*vel_fac)+' km s!U-1!N', box=0, charsize=plot_char
		;legend, 'Max median vel: '+int2str(max(v_meds)*vel_fac)+' km s!U-1!N', box=0, charsize=plot_char, /right
	endif
endelse
...
utplot, t_all[0:kins_counter[0]-1], a_all[0:kins_counter[0]-1]*accel_fac, utbasedata, psym=1, ytit='Acceleration (m s!U-2!N)', $
        xr=[xls,xrs], /xs, yr=[-accel_max_range,accel_max_range], /ys, pos=[0.15,0.05,0.95,0.33], /nodata; ,color=kins_color[0]
print, 'plotting 0 up to ', kins_counter[0]-1, ' as color ', kins_color[0]
outplot, t_all[0:kins_counter[0]-1], a_all[0:kins_counter[0]-1]*accel_fac, psym=1, color=kins_color[0]
if size(kins_counter,/dim) ne 0 then begin
	first_t = kins_counter[0]
	last_t = kins_counter[0]+(kins_counter[1]-1)
	for i=1,n_elements(kins_counter)-2 do begin
			...
			print, 'plotting ', first_t, ' up to ', last_t, ' as color ', kins_color[i]
			outplot, t_all[first_t:last_t], a_all[first_t:last_t]*accel_fac,psym=1,$
	        	        color=kins_color[i]
			first_t = last_t + 1
	        	last_t = first_t + (kins_counter[i+1]-1)
		endif else begin
	...
	print, 'final plotting ', first_t, ' up to ', n_elements(t_all)-1, ' as color ', kins_color[n_elements(kins_color)-1]
	outplot, t_all[first_t:n_elements(t_all)-1], a_all[first_t:n_elements(t_all)-1]*accel_fac, psym=1, color=kins_color[n_elements(kins_color)-1]
endif
loop_exit2:
...
if keyword_set(plot_quartiles) then begin
	outplot, t_bins, a_meds*accel_fac, utbasedata, psym=-3, thick=thick_t, color=c, line=0
	outplot, t_bins, a_q1s*accel_fac, utbasedata, psym=-3, thick=thick_t, color=c, line=2
	outplot, t_bins, a_q3s*accel_fac, utbasedata, psym=-3, thick=thick_t, color=c, line=2
	outplot, t_bins, lowerfence*accel_fac, utbasedata, psym=-3, thick=thick_t, color=c, line=2
	outplot, t_bins, upperfence*accel_fac, utbasedata, psym=-3, thick=thick_t, color=c, line=2
	...
	if keyword_set(plot_median) then begin
		outplot, t_bins, a_meds*accel_fac, utbasedata, psym=-3, thick=thick_t, color=c, line=0
		;horline, median(a_all)*accel_fac, line=4
		;legend, 'Median accel: '+int2str(median(a_all)*accel_fac)+' m s!U-2!N', box=0, charsize=plot_char
		;legend, 'Max median accel: '+int2str(max(a_meds)*accel_fac)+' m s!U-2!N', box=0, charsize=plot_char, /right
	endif
endelse
