...
; Last edited:	2013-01-16	JASON 
;		2013-05-01	to correct the residuals being added to the original fit (doesn't change the results).

pro plot_noise_variation_jpb_redo, weight = weight, plot_h = plot_h
...
; Simulated data  
  ...
  !p.multi = [0,1,3]
  set_plot, 'ps'
  if keyword_set(weight) then fname = 'noise_hist_weight_jpb_redo.eps' else fname = 'noise_hist_jpb_redo.eps'
...
; Set some constants for calculating the confidence intervals.
  ...
  if keyword_set(plot_h) then begin
     if keyword_set(weight) then fname = 'ht_cad_boot_weight_jpb.eps' else fname = 'ht_cad_boot_jpb.eps'
  endif else begin
     if keyword_set(weight) then fname = 'cad_boot_weight_jpb.eps' else fname = 'cad_boot_jpb.eps'
  endelse
...
; Calculate fitted line for r(t) vs t plot.     
  ...
  al_legend, [num2str(round(mult_noise))+'% scatter', num2str(round(delta_t))+'s cadence'], textcolor = [0,0], box=0, /top, $
             /left, charsize = chars_l
  al_legend, ['v!D0!N = '+num2str(round(float(f[1]*1e3)))+' !9+!3 '+num2str(round(float(p_err[1]*1e3)))+' km s!U-1!N (!9+!3 '+num2str(round(float(pcerr[1]*1e3)))+' km s!U-1!N  rescaled)', $
              'a = '+num2str(round(float(f[2]*1e6)))+' !9+!3 '+num2str(round(float(p_err[2]*1e6)))+' m s!U-2!N (!9+!3 '+num2str(round(float(pcerr[2]*1e6)))+' m s!U-2!N  rescaled)'], $
             textcolor = [0,0], box=0, /bottom, /right, charsize = chars_l
...
; Histograms for parameters - R0    
  ...
  if keyword_set(plot_h) then begin
     plot, xd, hd, color = 0, background = 255, charsize = chars, title = ' ', ytitle = 'Freq. of occurence', $
           ...
           yr = [0, 1.2*max([hd,hv,ha])], /ys, psym = 10
...
; Histograms for parameters - V0   
  if keyword_set(plot_h) then $
     plot, xv, hv, color = 0, background = 255, charsize = chars, title = ' ', psym = 10, $
           xr = [1e3*p2_b[0] - 3*1e3*s2_b, 1e3*p2_b[0] + 3*1e3*s2_b], /xs, yr = [0, 1.2*max([hd,hv,ha])], $
           /ys, xtit = '!3Velocity (km s!U-1!N)', pos = pos_v, ytitle = ' ', ytickname = [replicate(' ',10)] $
             ...
             xr = [1e3*p2_b[0] - 3*1e3*s2_b, 1e3*p2_b[0] + 3*1e3*s2_b], /xs, yr = [0, 1.2*max([hv,ha])], $
...
; Add 95% confidence interval lines  
  ...
  verline, mu[0]/height_factor, linestyle=1
  verline, conf_range[0]/height_factor, linestyle=5
  verline, conf_range[1]/height_factor, linestyle=5
  
  al_legend, ['95% CI: ['+num2str(round(conf_range[0]/height_factor))+', '+num2str(round(conf_range[1]/height_factor))+'] km s!u-1!n'], $
             textcolor = [0], box=0, /top, /left, charsize = chars_l, /clear
  
; Histograms for parameters - A     
  if keyword_set(plot_h) then $
     plot, xa, ha, color = 0, background = 255, charsize = chars, title = ' ', psym = 10, $
           xr = [1e6*p3_b[0] - 3*1e6*s3_b, 1e6*p3_b[0] + 3*1e6*s3_b], /xs, yr = [0, 1.2*max([hd,hv,ha])], $
           /ys, xtit = '!3Acceleration (m s!U-2!N)', pos = pos_a, ytitle = ' ', ytickname = [replicate(' ',10)] $
  else plot, xa, ha, color = 0, background = 255, charsize = chars, title = ' ', psym = 10, xtickinterval = 20, $
             xr = [1e6*p3_b[0] - 3*1e6*s3_b, 1e6*p3_b[0] + 3*1e6*s3_b], /xs, yr = [0, 1.2*max([hv,ha])], $
             /ys, xtit = '!3Acceleration (m s!U-2!N)', pos = pos_a, ytitle = ' ', ytickname = [replicate(' ',10)]
...
; Add 95% confidence interval lines
  ...
  al_legend, ['95% CI: ['+num2str(round(conf_range[0]/height_factor))+', '+num2str(round(conf_range[1]/height_factor))+'] m s!u-2!n'], $
             textcolor = [0], box=0, /top, /left, charsize = chars_l, /clear
