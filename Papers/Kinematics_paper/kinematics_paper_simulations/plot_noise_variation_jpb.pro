...
pro plot_noise_variation_jpb, weight = weight, plot_h = plot_h
...
; Simulated data  
  ...
  !p.multi = [0,1,3]
  set_plot, 'ps'
  if keyword_set(weight) then fname = 'noise_hist_weight_jpb.eps' else fname = 'noise_hist_jpb.eps'
...
; Calculate fitted line for r(t) vs t plot.     
  ...
  al_legend, ['v!D0!N = '+num2str(round(float(f[1]*1e3)))+' !9+!3 '+num2str(round(float(p_err[1]*1e3)))+' km s!U-1!N (!9+!3 '+num2str(round(float(pcerr[1]*1e3)))+' km s!U-1!N  rescaled)', $
              'a = '+num2str(round(float(f[2]*1e6)))+' !9+!3 '+num2str(round(float(p_err[2]*1e6)))+' m s!U-2!N (!9+!3 '+num2str(round(float(pcerr[2]*1e6)))+' m s!U-2!N  rescaled)', 'Model: v!D0!N = 400 km s!U-1!N, a = -150 m s!U-2!N'], $
             textcolor = [0,0,0], box=0, /bottom, /right, charsize = chars_l
