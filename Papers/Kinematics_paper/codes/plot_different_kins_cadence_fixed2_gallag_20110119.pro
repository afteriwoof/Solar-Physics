...
if ~keyword_set(no_plot) then begin
	...
	plots, fixed, h[where(tt eq fixed)]/height_factor, psym=6, color=6
	;if keyword_set(plot_fixed_point) then plots, fixed, h[where(tt eq fixed)]/height_factor, psym=5, color=6
endif
...
;	oplot, tt, deriv(tt, h)/vel_factor, psym=1, color=2
	...
	if keyword_set(plot_fixed_point) then plots, fixed, v[where(tt eq fixed)]/vel_factor, psym=5, color=6
endif
...
if ~keyword_set(no_plot) then begin
	...
	if keyword_set(plot_fixed_point) then begin
		...
		plots, fixed, a[where(tt eq fixed)]/accel_factor, psym=5, color=6
