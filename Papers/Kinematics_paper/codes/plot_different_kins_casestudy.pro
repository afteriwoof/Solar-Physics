...
pro plot_different_kins_casestudy, casestudy_file, cadence, fixed, no_plot=no_plot, fit_vel=fit_vel, large_range=large_range, af, ac, al, att, no_legend=no_legend
...
readcol, casestudy_file, smj, smn, tilt, aw, hc, ha, instr, date, time, h_err, delta_t, $
...
; Accel model:
yf = 'p[0]*x*atan(exp(x/p[1])/p[2])'
f = mpfitexpr(yf, t, ha, h_err, /quiet)
h_fit = f[0]*t*atan(exp(t/f[1])/f[2])
...
if ~keyword_set(no_plot) then begin
	plot, tt, h/1e6, psym=-0, linestyle=1, /ylog, xtit='Time (secs)', ytit='Height (Mm)'
	;oplot, tt, h_noisy/1e6, psym=1, color=3
	oploterror, tt, h_noisy/1e6, tt_sig, h_noisy_sig/1e6, psym=3
	if keyword_set(fit_vel) then begin
	...
	oplot, t, h_fullmodel/1e6, psym=-0
	plots, fixed, h[where(tt eq fixed)]/1e6, psym=5, color=6
endif
...
if ~keyword_set(no_plot) then begin
	if ~keyword_set(large_range) then begin
		plot, tt, v/1e3, psym=-0, linestyle=1, yr=[0,max(v/1e3)], xtit='Time (secs)', ytit='Velocity (km/s)'
	endif else begin
		plot, tt, v/1e3, psym=-0, linestyle=1, yr=[min(v_forward-v_forward_sig)/1e3,max(v_forward+v_forward_sig)/1e3], xtit='Time (secs)', ytit='Velocity (km/s)'
	endelse
...
;	oplot, tt, deriv(tt, h)/1e3, psym=1, color=2
	...
	oplot, t, v_fullmodel/1e3, psym=-0
	plots, fixed, v[where(tt eq fixed)]/1e3, psym=5, color=6
endif
...
if ~keyword_set(no_plot) then begin
	if ~keyword_set(large_range) then begin
		plot, tt, a, psym=-0, linestyle=1, yr=[-0.1,0.5], xtit='Time (secs)', ytit='Acceleration (m/s/s)'
	endif else begin
		plot, tt, a, psym=-0, linestyle=1, yr=[min(a_forward-a_forward_sig), max(a_forward+a_forward_sig)], xtit='Time (secs)', ytit='Acceleration (m/s/s)'
	endelse
	...
	plots, tt, a_lagrangian, psym=-1, color=4
	oploterror, tt, a_lagrangian, a_lagrangian_sig, psym=3, errcolor=4
	...
	oplot, t, a_fullmodel, psym=-0
	; Plotting the fixed point
	plots, fixed, a[where(tt eq fixed)], psym=5, color=6
	plots, fixed, a_forward[where(tt eq fixed)], psym=4, color=7
	plots, fixed, a_lagrangian[where(tt eq fixed)], psym=4, color=7
