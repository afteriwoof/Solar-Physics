; Created	20120414	to plot two example cadence acceleration plots, and the accel vs cadence plot below.

pro fig_cadence, tog=tog
...
if ~keyword_set(tog) then begin
        ...
        !p.charthick=3
	set_plot, 'ps'
	device, /encapsul, bits=8, language=2, /portrait, /color, filename='fig_cadence.eps', xs=15, ys=25
endelse
!p.multi=[0,1,3]
;Model;r0 = 100e6 ;m
;v0 = 400e3 ;m/s
...
if plot_no eq 1 then cadence=3 else cadence=10
...
if plot_no eq 1 then begin
	pos=[0.15,0.69,0.9,0.95]
endif else begin
	pos=[0.15,0.425,0.9,0.685]
endelse
...
if ~keyword_set(no_plot) then begin
        if plot_no eq 1 then plot, tt, a/accel_factor, psym=-0, linestyle=1, xr=xr, /xs, yr=yr, /ys, ytit='Accel. (m s!U-2!N)', xtickname=[' ',' ',' ',' ',' '], pos=pos $
		else plot, tt, a/accel_factor, psym=-0, linestyle=1, xr=xr, /xs, yr=yr, /ys, xtit='Time (mins)', ytit='Accel. (m s!U-2!N)', pos=pos
        ;plots, tt, a_lagrangian, psym=-1, color=4
        plots, tt, a_lagrangian/accel_factor, psym=-1;, color=5
        plots, tt[0], a_lagrangian[0]/accel_factor, psym=-1, color=5
        oploterror, tt, a_lagrangian/accel_factor, a_lagrangian_sig/accel_factor, psym=3;, errcolor=5
        oploterror, tt[0], a_lagrangian[0]/accel_factor, a_lagrangian_sig/accel_factor, psym=3, errcolor=5
	...
	legend, 'Cadence: '+int2str(cadence)+' mins', /right, box=0, charsize=1
endif
...
restore, 'cadence_vars.sav', /ver
num_x = indgen(num)+1
plot, num_x, att_array[1:*]/accel_factor, psym=-3, yr=yr, /ys, xtit='Cadence', ytit='Accel. (m s!U-2!N)', pos=[0.15,0.1,0.9,0.36]
...
;horline, att_array[1]/accel_factor, linestyle=1
;print, 'horline ', att_array[1]/accel_factor
