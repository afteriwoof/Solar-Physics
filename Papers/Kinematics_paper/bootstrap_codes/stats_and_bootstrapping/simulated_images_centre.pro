...
pro simulated_images_centre, date, cadence
...
; Images
	...
	toggle, /portrait, filename = 'stats_sim_c_1.eps', xsize = 6, ysize = 6, /inches, /color, bits_per_pixel = 8
...
; Calculate numerically differentiated data
	...
	d_y_f = sim_dist_195_quad[2:max(n)-1] - sim_dist_195_quad[0:max(n)-3]
	...
	d_y_f_e = v_195_deriv_quad[2:max(m)-1] - v_195_deriv_quad[0:max(m)-3]

	d_x_f_e = t_cadence[2:max(m)-1] - t_cadence[0:max(m)-3]
...
; Calculate numerically differentiated data
	...
	d_y_f = v_195_deriv_quad[2:max(m)-1] - v_195_deriv_quad[0:max(m)-3]
	...
	d_y_f_e = a_195_deriv_quad[2:max(c)-1] - a_195_deriv_quad[0:max(c)-3]

	d_x_f_e = t_cad[2:max(c)-1] - t_cad[0:max(c)-3]
...
; Constant a fit to acceleration data
	...
	uterrplot, t_cadence[1:max(m)-2], a_195_deriv_quad + mean(deltaa_195_deriv), a_195_deriv_quad - mean(deltaa_195_deriv), $
		thick = 2, color = 0
...
; Images
	...
	toggle, /portrait, filename = 'stats_sim_c_2.eps', xsize = 6, ysize = 6, /inches, /color, bits_per_pixel = 8
...
; Calculate numerically differentiated data
	...
	d_y_f = sim_dist_171_quad[2:max(n)-1] - sim_dist_171_quad[0:max(n)-3]
	...
	d_y_f_e = v_171_deriv_quad[2:max(m)-1] - v_171_deriv_quad[0:max(m)-3]
...
; Calculate numerically differentiated data
	...
	d_y_f = v_171_deriv_quad[2:max(m)-1] - v_171_deriv_quad[0:max(m)-3]
	...
	d_y_f_e = a_171_deriv_quad[2:max(c)-1] - a_171_deriv_quad[0:max(c)-3]
...
; Constant a fit to acceleration data
	...
	utplot, t_cadence[1:max(m)-2], a_171_deriv_quad, time171[0], timerange = [time171[0] - 60, max(time171) + 60], $
		...
		background = 1, color = 0, psym = 2, charsize = 1, xstyle = 1, ystyle = 1, thick = 2, $
		pos = [0.15, 0.1, 0.95, 0.37]
	uterrplot, t_cadence[1:max(m)-2], a_171_deriv_quad + mean(deltaa_171_deriv), a_171_deriv_quad - mean(deltaa_171_deriv), $
		thick = 2, color = 0
