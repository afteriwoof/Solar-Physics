pro bootstrap_lin_171, date, grt_dist_171, time171, h_err, dist_arr_171, f_171_lin, conf_range_r, $
...
; Known equation we want to recover (100 data points)
	...
	data_fit_171_linear, grt_dist_171, time171, h_err, dist_arr_171, f_171_lin, perror

	y = f_171_lin[0] + f_171_lin[1]*x
...
; Add Gaussian noise mean=0, sigma =1 of differing amplitudes to m and c params
	...
	cnoise = RANDOMN( Seed1, 100.0d , /DOUBLE, /NORMAL )*perror[0] + f_171_lin[0]
	mnoise = RANDOMN( Seed2, 100.0d , /DOUBLE, /NORMAL )*perror[1] + f_171_lin[1]
...
;		xyouts, 7.1, 90, 'Mean: '+string(p1[0])+', Sigma: '+string(s1), chars=1.2, color = 0
	...
	if (keyword_set(plot_on)) then begin
		...
		plots, [f_171_lin[1], f_171_lin[1]], [0, max(hv)], color=3
		plots, [p2[0], p2[0]], [0, max(hv)], color=5
;		verline, 0.5d, color=3
;		verline, p2[0], color=5, lines=2
		legend,['Actual - mean: '+num2str(f_171_lin[1]) + ', sigma: '+num2str(perror[1]), $
				'Calculated - mean: '+num2str(p2[0]) + ', sigma: '+num2str(s2)],colors=[3, 0], /right, $
				chars=1.2, textcolor = [3, 0], /clear

	endif
			
		k = linspc(f_171_lin[1] - 20, f_171_lin[1] + 20, 100)
...
; Print out some comparisons 
		...
		Print, 'Original line def y = ' + num2str(f_171_lin[1]) + '*x + ' + num2str(f_171_lin[0])
		print, 'R0 = ' + num2str(f_171_lin[0]) + ' Mm, v0 = ' + num2str(1000*f_171_lin[1]) + ' km/s'
