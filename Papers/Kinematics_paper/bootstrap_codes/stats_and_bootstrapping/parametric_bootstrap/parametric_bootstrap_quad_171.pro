...
pro parametric_bootstrap_quad_171, date, grt_dist_171, time171, h_err, dist_arr_171, f_171_quad, $
...
; Fitted equation that we wish to test.
	...
	if (keyword_set(plot_on)) then begin
		...
		data_fit_171_quadratic, grt_dist_171, time171, h_err, dist_arr_171, f_171_quad, perror, /plot_on
...
; Plot trend line and standard deviation margin
	...
	if (keyword_set(plot_on)) then begin
		...
		legend, ['R0 = ' + num2str(f_171_quad[0]) + ' +/- ' + num2str(perror[0]) + ' Mm', $
				'V0 = ' + num2str(1000*f_171_quad[1]) + ' +/- ' + num2str(1000*perror[1]) + ' km/s', $
				'A0 = ' + num2str(1e6*f_171_quad[2]) + ' +/- ' + num2str(1e6*perror[2]) + ' m/s/s'], $
...
; of fit to original data
		...
		dist_arr = RANDOMN( Seed1, dist_arr_171, /DOUBLE, /NORMAL )*perror[0] + f_171_quad[0]
...
; Plot histogram of mean distance parameter values
	...
	if (keyword_set(plot_on)) then begin
		...
		plot, loc_d, dist_hist, psym = 0, yr = [0, 1.25*max(dist_hist)], $
				...
				title = 'Bootstrapping analysis of 171A data R!D0!N fit parameter, ' + num2str(date)
...
; of fit to original data
		...
		vel_arr = RANDOMN( Seed2, dist_arr_171, /DOUBLE, /NORMAL )*perror[1] + f_171_quad[1]
...
; Plot histogram of mean distance parameter values
	...
	if (keyword_set(plot_on)) then begin
		...
		plot, loc_v, vel_hist, psym = 0, yr = [0, 1.25*max(vel_hist)], $
				...
				ytitle = 'Probability', thick = 2, $
				title = 'Bootstrapping analysis of 171A data V!D0!N fit parameter, ' + num2str(date)
...
; of fit to original data
		...
		acc_arr = RANDOMN( Seed3, dist_arr_171, /DOUBLE, /NORMAL )*perror[2] + f_171_quad[2]
...
; Plot histogram of mean distance parameter values
	...
	if (keyword_set(plot_on)) then begin
		...
		plot, loc_a, acc_hist, psym = 0, yr = [0, 1.25*max(acc_hist)], $
				...
				ytitle = 'Probability', thick = 2, $
				title = 'Bootstrapping analysis of 171A data A!D0!N fit parameter, ' + num2str(date)
