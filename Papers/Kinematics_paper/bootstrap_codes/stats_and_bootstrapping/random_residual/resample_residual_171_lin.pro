; Routine for the resampling residual bootstrapping method. 171A linear fit.

pro resample_residual_171_lin, date, grt_dist_171, time171, h_err, dist_arr_171, $
					f_171_lin, perror, p1, s1, p2, s2, conf_range_r, conf_range_v
...
; Zero a fit to data
    ...
    f_171_lin = mpfitexpr(lin_fit_171, tim171, grt_dist_171, h_error, [grt_dist_171[0], 0.2], $
...
; original fit to data
	...
	yfit = f_171_lin[1]*tim171 + f_171_lin[0]
...
; Create array of 1 and -1 to multiply ran array by to fill gaps in resulting array
		...
		unit_array = randomn(seed, dist_arr_171, /double, /normal)
...
; Plot original data with fitted line
	...
	plot, tim171, grt_dist_171, Title ='Original and noisy data - 171 Linear', xtit='X', ytit='Y', charsize=2.0, $
		background = 1, color = 0, xr = [-50, max(tim171) + 50], /xs, yr = [0, max(grt_dist_171) + 50], /ys, $
		psym = 2
	oplot, x_arr, y_line, color=3
	legend,['Data', 'Fit: r = '+ num2str(f_171_lin[0]) + ' + ' + num2str(f_171_lin[1]) + '*x'], $
