pro plot_hist2, data, x, h, bin=bin, xrange=xrange, yrange=yrange, $
   ...
   color=color,nodisp=nodisp,density=density,background=background, $
   position=position, xticks=xticks
;
;+
;NAME:
;	plot_hist
...
IF KEYWORD_SET(density) THEN h = float(h) / N_ELEMENTS(data)


n = n_elements(h)
amin = min(data0)
x = findgen(n)*bin + long(amin)*bin + bin/2.
;
...
if (n_elements(ytitle) eq 0) then ytitle='Freq. of Occurrences' else ytitle=ytitle

;

IF keyword_set(nodisp) THEN GOTO, notplotting
...
if (keyword_set(oplot)) then begin
	...
	xrange=xrange, yrange=[0, max(h)], title=title, ytype=keyword_set(log), $
	noerase=noerase, linestyle=linestyle, charsize=charsize, $
	xstyle=xstyle, ystyle=ystyle, xticklen=xticklen, yticklen=yticklen, $
	xmargin=xmargin, ymargin=ymargin, color=color, background=background, $
	position=position, xticks=xticks
end
;
