; Sub-routine called by kinematics_simulation if /vary_noise keyword set. This routine works out the
; variation of the fit from the actual for a set cadence and varying noise-level.

pro kinematics_simulation_noise
...
; Array to hold numbers for means of distance, velocity and acceleration

	mean_0 = fltarr(201)
	mean_1 = fltarr(201)
	mean_2 = fltarr(201)

	mean_scatter = fltarr(201)
...
; Mean percentage difference of the cadence & noise affected data points from the fit to the data

	scatter_dist = fltarr(201)
...
; Difference between model and fit
...
; Percentage mean of difference between data points and fit to data points

		scatter_dist[i] = mean((abs(h_obs - dist_fit)/h_obs)*100.)
...
;		print, '************'
;		print, 'Percentage difference between acceleration data fit and actual = ', percent_diff_acc[noise_level]
;		print, '************'
;		print, ' '
	...
	mean_scatter[j] = mean(scatter_dist)

; X-window to png

		IF j LE 9 THEN BEGIN
			s = arr2str(j, /trim)
			x2png, 'iteration_00' + s +'.png'
		ENDIF

		IF (j GT 9) AND (j LE 99) THEN BEGIN
			s = arr2str(j, /trim)
			x2png, 'iteration_0' + s + '.png'
		ENDIF

		IF j GT 99 THEN BEGIN
			s = arr2str(j, /trim)
			x2png, 'iteration_' + s + '.png'
		ENDIF
	...
	plot, iterations, mean_0, xtitle = '!6% Noise level', ytitle = '!6Mean percentage difference from fit', $
			title = '!6Variation of fit to distance data with iterations (Cadence = 150s)', charsize = 2.5
	plot, iterations, mean_1, xtitle = '!6% Noise level', ytitle = '!6Mean percentage difference from fit', $
			title = '!6Variation of fit to velocity data with iterations', charsize = 2.5
	plot, iterations, mean_2, xtitle = '!6% Noise level', ytitle = '!6Mean percentage difference from fit', $
			title = '!6Variation of fit to acceleration data with iterations', charsize = 2.5

	x2png, 'kinematics_sim_cadence_150s.png'
	
	!p.multi = [0, 1, 1]

	window, 1, xs = 600, ys = 400
	
	plot, iterations, mean_scatter, xtitle = '!6% Noise level', ytitle = '!6Mean percentage scatter in data', $
			title = '!6Scatter of data with respect to fit(Cadence = 150s)', charsize = 1.5

	save, filename = 'kinematics_sim_cadence_150.sav', iterations, mean_0, mean_1, mean_2, mean_scatter
