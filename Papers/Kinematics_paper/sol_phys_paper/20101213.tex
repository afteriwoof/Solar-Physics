...
Due to the approximation of an infinite series with finite iterations, a truncation error must be associated with the result, based on its deviation from the true solution.
...
\end{equation}
where $O(\Delta t)$ is the truncation error term, proportional to the step size $\Delta t$ between datapoints. %This technique assumes a straight line gradient between points. 
...
\end{equation}
The truncation error in this case is proportional to the square of the step size $\Delta t$ between datapoints. %, and thus has greater precision than the forward and reverse difference methods. 
