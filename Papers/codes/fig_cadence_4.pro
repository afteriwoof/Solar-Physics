; Created	20120417	to plot 4 figures, two cases of cadence and two cases of fixed points.

; Last edited	2012-07-12	to take in the new output from plot_peakaccel_cadence_fixedpoint.pro as called in peak_a_change_cadence.b


pro fig_cadence_4, tog=tog
...
if plot_no eq 1 then begin
	pos=[0.15,0.75,0.9,0.95]
endif else begin
	pos=[0.15,0.545,0.9,0.745]
endelse
...
if ~keyword_set(no_plot) then begin
        if plot_no eq 1 then plot, tt, a/accel_factor, xr=xr, /xs, yr=yr, /ys, ytit='Accel. (m s!U-2!N)', xtickname=[' ',' ',' ',' ',' ',' ',' '], pos=pos,/nodata $
		else plot, tt, a/accel_factor, xr=xr, /xs, yr=yr, /ys, xtit='Time (s)', ytit='Accel. (m s!U-2!N)', pos=pos, /nodata
	...
	legend, 'Cadence: '+int2str(cadence)+'s', /right, box=0, charsize=1
...
plot, xnum, al_array[ind]/accel_factor, xr=[0,num], /xs, yr=[-600,1200], /ys, ytit='Accel. (m s!U-2!N)', /nodata, $
	xtickname=[' ',' ',' ',' ',' ',' ',' '], pos=[0.15,0.3,0.9,0.5]
...
plot, xnum, al_array[ind]/accel_factor, xr=[0,num], /xs, yr=[-600,1200], /ys, xtit='Cadence (s)', $
	ytit='Accel. (m s!U-2!N)', /nodata, pos=[0.15,0.095,0.9,0.295]
horline, att_array[1]/accel_factor, line=3, thick=1
horline, a_max_array[1]/accel_factor, line=2, thick=1
horline, 0, line=0, thick=1
oplot, xnum, al_array[ind]/accel_factor, psym=1, color=5
