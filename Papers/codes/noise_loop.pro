...
pro noise_loop, weight = weight, plot=plot, tog = tog
...
; Original fit to data     
        ...
        fit = mpfitexpr(fit_model, t[0:*:delta_t], sim_d[0:*:delta_t], [1., 0.2, 0.00005], perror=perror, $
                        bestnorm = bestnorm, /quiet, /weight)
...
; New fit and store the results        
           fit = mpfitexpr(fit_model, x_r, y_r, [1., 0.2, 0.00005], perror=perror, bestnorm = bestnorm, /quiet, /weight)
           res_b[*,i] = fit
