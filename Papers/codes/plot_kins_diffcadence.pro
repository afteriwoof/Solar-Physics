; Code to plot the different kinematics generate on a noised data set compared with the model, for different cadences along the sample (c.f. different instruments).

;Created: 20-01-2011

; Last edited: 20-01-2011

pro plot_kins_diffcadence, cadence, fixed, no_plot=no_plot, large_range=large_range, al, att, a_max, no_legend=no_legend, plot_fixed_point=plot_fixed_point, tog=tog
...
if ~keyword_set(no_plot) then begin
	if ~keyword_set(large_range) then begin
		...
		plot, tt, v/vel_factor, psym=-0, linestyle=1, yr=[min(v_lagrangan-v_lagrangian_sig)/vel_factor,max(v_lagrangian+v_lagrangian_sig)/vel_factor], xr=[-10,210], /xs, /ys, xtit='Time (secs)', ytit='Velocity (km/s)'
	endelse
...
if ~keyword_set(no_plot) then begin
	if ~keyword_set(large_range) then begin
		...
		plot, tt, a/accel_factor, psym=-0, linestyle=1, yr=[min(a_lagrangian/accel_factor-a_lagrangian_sig/accel_factor), max(a_lagrangian/accel_factor+a_lagrangian_sig/accel_factor)], xr=[-10,210], /xs, /ys, xtit='Time (secs)', ytit='Acceleration (m/s/s)'
	endelse
