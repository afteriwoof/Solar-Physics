; Created	2013-06-07	from project_stereo_to_soho.pro to read in the intersects.txt coords from project_soho_to_stereo_measure.pro.
...
pro project_stereo_to_soho_measure_readin, in_soho, in_stereo, da_soho_ell, da_stereo_ell, pos1, pos2, pos3, pos4, plot3d=plot3d, zoom=zoom, system=system, behind=behind
...
; If x,y,z are entered in pixel values on STEREO POS:
y = ((y_in-in_stereo.crpix1)*in_stereo.cdelt1)/in_stereo.rsun ; rsun
z = ((z_in-in_stereo.crpix2)*in_stereo.cdelt2)/in_stereo.rsun ; rsun
...
; First the STEREO coodes in HEEQ:
...
; Find the intersection point of the line-of-sight from SOHO to STEREO through the SOHO plane-of-sky.
...
; Loop through the equations for each epipolar line to find where the intersections that are read in from STEREO cross the ellipse-fit in the SOHO image.
openw, lun_intersects3, /get_lun, 'intersects3.txt'
free_lun, lun_intersects3
openw, lun_intersects4, /get_lun, 'intersects4.txt'
free_lun, lun_intersects4
qtemp_intersect1 = 0
qtemp_intersect2 = 0
temp_count = 0
temp_sz = size(da_soho_ell,/dim)
for i=0,n_elements(y_epi)-1 do begin
	for temp_x=0,temp_sz[0]-1 do begin
		temp_y = temp_x*m_epi[i] + c_epi[i]
		if da_soho_ell[temp_x,temp_y] eq 888 then begin
	...
	if qtemp_intersect1 eq 1 then plots, temp_intersect1, psym=2, color=4
	if qtemp_intersect2 eq 1 then plots, temp_intersect2, psym=2, color=4
	if qtemp_intersect1 eq 1 AND qtemp_intersect2 eq 1 then begin
		openu, lun_intersects3, 'intersects3.txt', /append
		printf, lun_intersects3, temp_intersect1[0], temp_intersect1[1]
		free_lun, lun_intersects3
		openu, lun_intersects4, 'intersects4.txt', /append
		printf, lun_intersects4, temp_intersect2[0], temp_intersect2[1]
		free_lun, lun_intersects4
	endif
endfor

print, 'Writing out intersects3.txt and intersects4.txt'
