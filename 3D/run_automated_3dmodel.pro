; Created	20131115	from run_automated_new.pro to be used specifically on Huw's 3D model.
...
pro run_automated_3dmodel, fls, out_dir, pa_total, gail=gail, stereo=stereo, behind=behind, old_data=old_data, show=show, overwrite=overwrite
...
for count=0,n_elements(fls)-1 do begin
	...
	;da = readfits_corimp(fls[count],in) <--- was giving me NaNs that mreadfits_corimp doesn't for some reason!
...
;	endif
	...
	n = randomn(seed,in.naxis1,in.naxis2)
	da += n
