; Created	2013-08-06	from plot_kins_quartiles.pro to take in the R_Sun height values instead of arcsecs.
...
for i=0L,n_elements(heights)-1 do heights_km[i] = heights[i]*6.955e5
...
if keyword_set(debug) then begin
	print, 'utbasedata: ', anytim(utbasedata,/truncate,/ccsds)
	print, 'filename: ', out_dir+'/plot_kins_quartiles_'+plot_name+'_ps_name_'+lon_str+'.eps'
endif
...
if keyword_set(tog) AND ~keyword_set(debug) then begin
	...
	ps_name = strjoin(strsplit(ps_name,'-',/extract))
	device, /encapsul, bits=8, language=2, /portrait, /color, filename=out_dir+'/plot_kins_quartiles_'+plot_name+'_'+ps_name+'_'+lon_str+'.eps', xs=20, ys=30
        !p.background=1
        !p.charsize=2.0
        !p.charthick=5
        !p.thick=3
...
for i=0L,n_elements(datetime)-2 do begin
	if datetime[i+1] gt datetime[i] then v[i] = (heights_km[i+1]-heights_km[i])/(time[i+1]-time[i])
endfor
...
if keyword_set(saves) then begin
	...
	save, heights_km, v, a, time, f='kins_temp.sav'
endif
infvar = 3./0.
...
utplot, t, (heights_km/1000.), utbasedata, psym=1, ytit='Height (Mm)', pos=[0.15,0.63,0.92,0.91],yr=y_range, $
	...
	xtit='',tit='', ystyle=9;, tit=title;color=3
...
if keyword_set(model) then begin
		...
		;if ~keyword_set(old_data) then horline, 25.*695.5, linestyle=2 else horline, 20*695.5, linestyle=2
        endelse
endelse
...
cme_kins = {Event_StartTime:datetime[0], Event_EndTime:datetime[n_elements(datetime)-1], CME_RadialLinVel:(median(v_q1s)), $
	CME_RadialLinVelMin:(median(v_q3s)), CME_RadialLinVelMax:(median(v_upperfence)), CME_RadialLinVelStddev:(median(v_q1s)-mean(v_all)), $
	CME_RadialLinVelUnit:'km/s', CME_CPA:(min(pos_angles)+0.5*(max(pos_angles)-min(pos_angles))), CME_PosAng1:min(pos_angles), CME_PosAng2:max(pos_angles), CME_AngularWidth:(max(pos_angles)-min(pos_angles)), CME_AngularWidthUnit:'deg', $
	CME_Accel:(median(a_q1s)*1000.), CME_AccelMax:(median(a_upperfence)*1000.), CME_AccelMin:(median(a_q3s)*1000.), CME_AccelUnit:'m/s/s'}

out_name = anytim(utbasedata,/truncate,/ccsds)
out_name = strjoin(strsplit(out_name,':',/extract))
out_name = strjoin(strsplit(out_name,'-',/extract))

save, cme_kins, filename=out_dir+'/cme_kins_'+plot_name+'_'+out_name+'_'+lon_str+'.sav'

; temporary save files
if dir_exist('cme_profs') eq 0 then spawn, 'mkdir -p cme_profs'
save, new_t, new_heights, utbasedata, t_all, kins_counter, v_all, a_all, kins_color, pos_angles, f=out_dir+'/../cme_profs/kins_time_'+lon_str+'.sav'
save, t_bins, utbasedata, v_meds, v_q1s, v_q3s, v_lowerfence, v_upperfence, a_meds, a_q1s, a_q3s, a_lowerfence, a_upperfence, f=out_dir+'/../cme_profs/interquartiles_'+lon_str+'.sav'

;fl_xml = file_search(out_dir+'/cme_kins_'+plot_name+'_'+out_name+'.sav')
;dir_xml = strmid(out_dir,0,strlen(out_dir)-14)

;ce_xml, fl_xml, dir_xml
