...
; Created 27-08-09
; Created 26-01-10 from kins_figure3.pro

; Last Edited: 15-09-09  to plot only the Flanks and Middle of CME.

; combining kins_figure1 and kins_figure2.

; Last edited: 09-12-09 taken from kins_figure3.pro in paper_figures folder under combining/.
; Last edited: 02-03-10 to include the prominence heights overplot on deflection.
; Last Edited: 21-04-10 from kins_figure to try and be more general in its application.

; Read in 'kins_meanspread.txt' as file

; Best to run this code as kins_figure4,'kins_meanspread.txt',/edges,/occulter,/mids,/tog,/heights

; copied over from original kins_figure4.pro now in redone/.



pro kins_3d, file, edges=edges, tog=tog, heights=heights, occulter=occulter, arc_length=arc_length, mids=mids, prominence=prominence, occult_widths=occult_widths
...
if keyword_set(tog) then begin
	toggle, f='kins_3d.ps', /color, /portrait
	!p.charsize=2
	!p.charthick=3
	!p.thick=3
endif
...
if ~keyword_set(heights) then begin
	...
	if ~keyword_set(edges) then begin
		...
		plotsym, 5;, /fill
		plot, indgen(10), /nodata, ytit='!3Declination !9q !3(deg)', xr=[height_plot_start,height_plot_end], yr=[deflect_lower,deflect_upper], $
			/xs, /ys, xtickname=[' ',' ',' ',' ',' '], pos=[0.15,0.23,0.95,0.58], $
			charsize=2.2, xthick=3, ythick=3, color=0
		oplot, bottom_r, bottom_a, psym=8, linestyle=1, color=9
		horline, 0, linestyle=0, thick=1
		plotsym, 0, /fill
		oplot, mid_r, mid_a, psym=8, linestyle=1, color=5
		print, 'mid_r ', mid_r & print, 'mid_a ', mid_a
		plotsym, 4;, /fill
		oplot, top_r, top_a, psym=8, linestyle=1, color=3

		if keyword_set(mids) then begin
			plotsym, 5, /fill
			oplot, midbottom_r, midbottom_a, psym=8, color=9
			plotsym, 4, /fill
			oplot, midtop_r, midtop_a, psym=8, color=3
		endif

		; DETERMINE A FIT TO THE DEFLECTION
		
		;oploterror, mid_r, mid_a, h_errs, aw_errs, psym=3, color=0

		yf3 = 'p[0]*x^p[1]'
		;f3 = mpfitexpr(yf3, mid_r, mid_a, aw_errs, perror=perror)
		f3 = mpfitexpr(yf3, mid_r, mid_a, perror=perror)
		model3 = f3[0]*dindgen(50)^f3[1]
		help, model3
		print, 'A power law deflection fit gives '
		print, f3[0], ' h^', f3[1]
		...
		plotsym, 0, /fill
		legend, ['!9q!3 = '+string(f3[0],f='(I2.0)')+'(r/R'+sunsymbol()+')!U'+string(f3[1],f='(D0.1)')], $
			psym=-8, color=5, linestyle=1, charsize=1.1, box=0, pos=[30,50];, /right, /top;, /bottom
		...
		oplot, dindgen(50), model3, psym=-3, linestyle=1
		...
		plotsym, 5;, /fill
		plot, indgen(10), /nodata, ytit='!3Declination !9q !3(deg)', xr=[0,48], yr=[-35,55], $
			/xs, /ys, xtickname=[' ',' ',' ',' ',' '], pos=[0.15,0.23,0.95,0.58], $
			charsize=2.2, xthick=3, ythick=3, color=0
