...
; This reads in the points taken from my_scc_measure.pro and computes the corresponding points in first image.

; Last Edited: 12-02-09
...
;  the color table.
;
common my_scc_measure_readin, ahead, h_ahead, behind, h_behind, wcs_ahead, wcs_behind, $
...
;  the selected point.
;
openw, lun_intersects3, /get_lun, 'intersects3.txt', error=err
free_lun, lun_intersects3, f='(D)'
openw, lun_intersects4, /get_lun, 'intersects4.txt', error=err
free_lun, lun_intersects4, f='(D)'

;read in the points from the output of the my_scc_measure.pro ; intersects1.txt and intersects2.txt
...
if true eq 0 then begin
	my_pix = fltarr(2,n_elements(i00))
	my_pix[0,*]=i00
	my_pix[1,*]=i01
	sz_my_pix = size(my_pix, /dim)
	pix_1st = [my_pix[0,my_count_1], my_pix[1,my_count_1]]
endif else begin
	my_pix = fltarr(2,n_elements(i11))
	my_pix[0,*]=i10
	my_pix[1,*]=i11
	sz_my_pix = size(my_pix, /dim)
	pix_1st = [my_pix[0,my_count_2], my_pix[1,my_count_2]]
endelse
...
print, 'my_slope & my_intercept ', my_slope, my_intercept
qtemp_intersect1 = 0
qtemp_intersect2 = 0
temp_count = 0
temp_sz = size(behind,/dim)
...
print, my_count_1, my_count_2
my_count_1 += 1
if my_count_1 lt sz_my_pix[1] then goto, jump1
true = 1
my_count_2 += 1
if my_count_2 lt sz_my_pix[1] then goto, jump1
...
;==============================================================================
;
pro my_scc_measure_readin, file_ahead, file_behind, index_ahead, index_behind, $
