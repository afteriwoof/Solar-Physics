; Code to read in the CME front and fit an ellipss, taking out the kinematics along a particular radial line.
...
; taken from front_ell_kinematics_nonflip.pro

pro front_ell_kins_along_angle, image, errs, in, da, ang, along_angle, ell_info, r_err, x_ellipse, y_ellipse, perror, max_h_front, apex_a
	...
	; Take the furthest point on the ellipse ALONG ANGLE from Sun centre
	recpol, x_ellipse, y_ellipse, r_ellipse, a_ellipse, /degrees
	apex_a = a_ellipse[where(abs(a_ellipse-along_angle) eq min(abs(a_ellipse-along_angle)))]
	apex_a1 = apex_a[0]
	apex_r1 = max(r_ellipse[where(a_ellipse eq apex_a1)])
	temp1 = abs(a_ellipse-along_angle)
	temp2 = temp1[sort(temp1)]
	apex_a2 = a_ellipse[where(temp1 eq temp2[1])]
	apex_r2 = max(r_ellipse[where(a_ellipse eq apex_a2[0])])
	if (apex_r2 gt apex_r1) then begin 
		apex_a = apex_a2[0] 
		apex_r = apex_r2[0]
	endif else begin
		apex_a = apex_a1[0]
