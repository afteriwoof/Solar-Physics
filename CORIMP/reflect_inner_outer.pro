...
;		24-08-11	to include keyword model.
;		20110922	to include stereo keyword for secchi/cor data.
;		20120326	to include behind keyword for different FOV
;		2011-11-16	to just read in the thr_inner and thr_outer and delete the other keywords.
;		2013-05-14	to include new stereo processing.

function reflect_inner_outer, in, da, thr_inner, thr_outer, stereo=stereo, behind=behind;, model=model
...
while count lt 2 do begin
	...
	if in.i3_instr eq 'C2' || in.i3_instr eq 'c2' then begin
		npa = 900.
		case count of
		0:	htra = [thr_inner,thr_inner*2.];range of reflection in Rs. I go right into center just for fun.
		1:	htra = [thr_outer-(96.*pix_size),thr_outer]
		endcase
	endif
	if in.i4_instr eq 'C3' || in.i4_instr eq 'c3' then begin
		npa = 360.
		case count of
		0:	htra = [thr_inner, thr_inner*2.]
		1:	htra = [thr_outer-(96.*pix_size),thr_outer]
		endcase
	endif
