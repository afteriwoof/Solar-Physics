...
; Created: 23-04-09 from front_ell_kinematics.pro to limit the size of the ellipse.

pro front_ell_limited, image, errs, in, da, ang, ell_info, r_err, x_ellipse, y_ellipse, perror, max_h_front, max_h_ell, apex_a, noplot, noflank=noflank
	...
	parinfo = {value:0.D, fixed:1, limited:[1,1], limits:[0.D,0]}
	parinfo = [parinfo, replicate({value:0.D, fixed:0, limited:[0,0], limits:[0.D,0]}, 2)]
	parinfo[0].limits = [0, max_h_front]
	print, width
	print, max_h_front
	parinfo[*].value = [width, width, 1.]
	...
	; Putting the limiting constraints on here / limited ellipse fit

	parinfo[*].value = [param[0], param[1], x_bar, y_bar, param[2]]
	parinfo[0].limited[1] = 1
	parinfo[0].limits[1] = width  ; was divided by 2 as previous constraint but deleted here
