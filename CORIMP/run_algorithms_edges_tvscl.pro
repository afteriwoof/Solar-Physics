...
;		20110922	to include keyword stereo for reading in secchi/cor data.
;		20111115	edited keyword dynamic to non_dynamic so the default is a dynamic run.
...
pro run_algorithms_edges_tvscl, fls, pa_total, out_dir, saves0=saves0, show=show, pauses=pauses, no_png=no_png, no_reflect=no_reflect, gail=gail, non_dynamic=non_dynamic, model=model, stereo=stereo
...
while count lt n_elements(fls) do begin & $
	...
	in.filename = strmid(file_basename(fls[count]),str_count,12)
	...
	if ~keyword_set(no_reflect) then begin
		if keyword_set(model) then begin
			da=reflect_inner_outer(in, da, /model)
		endif else begin
			if keyword_set(stereo) then da=reflect_inner_outer(in,da,/stereo) else da=reflect_inner_outer(in,da)
		endelse
	endif
	...
	if in.i4_instr eq 'COR2' || in.i4_instr eq 'cor2' then begin
		if ~keyword_set(gail) then print, 'SECCHI/COR2'
		thr_inner = replicate(C2_thr_inner,4)
		thr_outer = replicate(C2_thr_outer,4)
	endif
	...
	if keyword_set(show) then begin
		if ~keyword_set(non_dynamic) then begin
			...
			cme_detection_mask_corimp, in, da, modgrad[*,*,3:6], alpgrad[*,*,3:6], cme_mask, list, $
				/show, /no_wait, /loud;, circ_pol, cme_detect, cme_detection_frames, /show, /no_wait, /loud
		endelse
	...
	if ~keyword_set(no_png) && ~keyword_set(gail) then begin
		if in.i3_instr eq 'C2' || in.i3_instr eq 'c2' then save, cme_mask, f= out_dir+'/CME_mask_'+strmid(in.filename,0)+'_C2.sav'	
		if in.i4_instr eq 'C3' || in.i4_instr eq 'c3' then save, cme_mask, f= out_dir+'/CME_mask_'+strmid(in.filename,0)+'_C3.sav'	
		if in.i4_instr eq 'COR2' || in.i4_instr eq 'cor2' then save, cme_mask, f= out_dir+'/CME_mask_'+strmid(in.filename,0)+'_COR2.sav'	
		print, 'Saving cme_mask'	
	endif
	...
	print, '***** Thresholding max_cme_mask = 3 for C2, C3, and COR2 *****'
	...
	if in.i4_instr eq 'COR2' || in.i4_instr eq 'cor2' then max_cme_mask = 3
...
;		if ~keyword_set(no_png) then if in.i3_instr eq 'C2' || in.i3_instr eq 'c2' then x2png, out_dir+'/CME_front_ell_'+count_label+'_C2_tvscl.png'
		...
		if ~keyword_set(no_png) && ~keyword_set(gail) then begin
			...
			if in.i4_instr eq 'COR2' || in.i4_instr eq 'cor2' then x2png, out_dir+'/CME_front_edges_'+strmid(in.filename,0)+'_COR2.png'	
		endif
...
print, 'Shifting pa_total so that it starts at solar north.'
temp = pa_total
...
save, pa_total, f=out_dir+'/pa_total_'+in.filename+'.sav'
