...
while count lt n_elements(fls) do begin & $
	...
	if in.i3_instr eq 'C2' then begin
		if ~keyword_set(gail) then print, '%' & print, 'LASCO/C2'
		if keyword_set(no_reflect) then begin
	...
	if in.i4_instr eq 'C3' then begin
		if ~keyword_set(gail) then print, '%' & print, 'LASCO/C3'
		if keyword_set(no_reflect) then begin
	...
	if in.i4_instr eq 'C3' then begin
		if ~keyword_set(gail) then print, 'Removing the C3 pylon region of the modgrad and alpgrad images'
	...
	if keyword_set(show) then begin
		...
		if ~keyword_set(gail) then print, 'Running cme_detection_mask_corimp'
