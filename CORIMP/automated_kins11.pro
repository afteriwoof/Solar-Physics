...
pro automated_kins11, debug=debug, _extra=_extra
...
for i_years=0,0 do begin
	months = file_search(years[i_years]+'/*')
        ...
        for i_months = 11,11 do begin
			...
			fls = file_search('~/Postdoc_largefiles/'+current_year+'/'+current_month+'/'+current_day+'/*dynamics*fits*')
			...
			separate_pa_total, pa_total, det_info, debug=debug, _extra=_extra
...
;				endfor
				...
				while i le count_days do begin
					...
					if strlen(file_search(next_dir)) eq 0 then begin
						det_fls = file_Search(dir+'/cme_dets/dets*')
                                		stack_fls = file_search(dir+'/det_stack*')
					endif else begin
						if n_elements(file_search('~/Postdoc_largefiles/'+next_date+'/*dynamics*fits*')) gt 1 then begin
							fls = [fls, file_search('~/Postdoc_largefiles/'+next_date+'/*dynamics*fits*')]
						endif else begin
							if strlen(file_search('~/Postdoc_largefiles/'+next_date+'/*dynamics*fits*')) ne 0 then fls = [fls, file_search('~/Postdoc_largefiles/'+next_date+'/*dynamics*fits*')]
						endelse
						if i eq 1 then det_fls = [file_search(dir+'/cme_dets/dets*'), file_search(next_dir+'/cme_dets/dets*')] $
							else det_fls = [det_fls, file_search(next_dir+'/cme_dets/dets*')]
...
;					plot_kins_quartiles, datetimes, heights, angs, dir+'/cme_kins_plots', /linear, /plot_quartiles, /tog, debug=debug
;					plot_kins_quartiles, datetimes, heights, angs, dir+'/cme_kins_plots', /quadratic, /plot_quartiles, /tog, debug=debug
					plot_kins_quartiles, datetimes, heights, angs, dir+'/cme_kins_plots', /sav_gol, /plot_quartiles, /tog, debug=debug
					...
					gather_gail_detections_inset_all, fls, det_fls, datetimes_in, angs_init, dir+'/cme_pngs_'+min(datetimes_in);, /old_data
