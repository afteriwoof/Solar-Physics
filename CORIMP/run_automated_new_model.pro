...
; Created	20120326	to take in the multicme_model fls.
...
pro run_automated_new_model, fls, out_dir, pa_total, gail=gail, stereo=stereo, behind=behind
...
if ~keyword_set(stereo) then begin
	...
	if keyword_set(behind) then C2_thr_outer = 11d else C2_thr_outer = 11d
	;if keyword_set(gail) then str_count = 9 else str_count=9
	if keyword_set(behind) then str_count = 1 else str_count = 26
endelse
...
while count lt n_elements(fls) do begin
	...
	if keyword_set(stereo) then in.filename = strmid(file_basename(fls[count]), str_count, 12) $
	...
	if ~keyword_set(gail) then $
		run_automated2_new, fls[count], in, da, multmods, cme_mask_thr, edges, out_dir, pa, heights, fail $
		else $
		run_automated2_new, fls[count], in, da, multmods, cme_mask_thr, edges, out_dir, pa, heights, fail, /gail
