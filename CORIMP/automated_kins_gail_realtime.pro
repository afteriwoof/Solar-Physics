; Created	2014-03-31	from automated_kins.pro

; Last edited:	2014-04-01	to take in statedate and enddate.
;		2014-05-20	to include keyword 'daily'
;		2014-06-05	to include keyword 'weekly'
;		2014-06-11	to include keyword 'latest'
;		2014-06-12	to edit the overwrite bit.

pro automated_kins_gail_realtime, startdate, enddate, debug=debug, originals=originals, overwrite=overwrite, daily=daily, weekly=weekly, latest=latest

if keyword_set(debug) then print, '*** automated_kins_gail_realtime.pro ***'

path = '/home/gail/jbyrne/realtime/soho/lasco/detections'
if keyword_set(daily) then path='/home/gail/jbyrne/realtime/soho/lasco/detections/daily'
if keyword_set(weekly) then path='/home/gail/jbyrne/realtime/soho/lasco/detections/weekly' 
if keyword_set(latest) then path='/home/gail/jbyrne/realtime/soho/lasco/detections/latest' 
fits_dir = '/home/gail/jbyrne/realtime/soho/lasco/separated/fits/'
...
for i_days=0,cntdir-1 do begin
	...
	if strlen(kin_fls[0]) eq 0 then goto, jump4
	...
	for i=0,n_elements(kin_fls)-1 do begin 
		...
		if n_elements(datetimes[uniq(datetimes,sort(datetimes))]) ge 3 then begin
			...
			plot_kins_quartiles_gail, datetimes_clean, heights_clean, angs_clean, out_dir+'/cme_kins', /linear, /plot_quartiles, /tog, debug=debug, /no_legend
