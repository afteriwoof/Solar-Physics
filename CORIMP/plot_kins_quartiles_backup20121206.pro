...
pro plot_kins_quartiles, datetime, heights, pos_angles, new_heights, new_pos_angles, gather_t_nows, gather_h_nows, gather_v_nows, gather_a_nows, tog=tog, plot_quartiles=plot_quartiles, plot_median=plot_median, remove_outliers=remove_outliers, saves=saves, model=model, main_angs=main_angs, sav_gol=sav_gol
...
utplot, t, (heights_km/1000.), utbasedata, psym=1, ytit='Height (Mm)', pos=[0.15,0.60,0.95,0.88],yr=[0,2.5e4],/ys, $
	xr=[t[0]-xls,t[n_elements(t)-1]+xrs], /xs, /nolabel , xtickname=[' ',' ',' ',' ',' ',' ',' '], /nodata;color=3
...
for i=min(pos_angles),max(pos_angles) do begin
	...
	if cnt gt 1 then begin
		...
		if keyword_set(sav_gol) AND n_elements(t_now) ge 6 then begin
