...
; Last Edited: 25-02-08

pro kins_curves_incl_cdaw_intervals
...
v = fltarr(n_elements(h)-1)
for k=0,n_elements(h)-2 do v[k]=(h[k+1]-h[k])/(t[k+1]-t[k])
a = fltarr(n_elements(h)-2)
for k=0,n_elements(h)-3 do a[k]=(v[k+1]-v[k])/(t[k+1]-t[k])
...
utplot, t, h_rsun, utbasedata, psym=3, linestyle=1, ytit='!6Height (R!D!9n!N!X)', $
	xr=[t[0]-2000,t[*]+1000], yr=[0,22], /xs, /ys, /notit, /nolabel, $
	xtickname = [' ',' ',' ',' ',' ',' ',' '], $
...
legend, ['CDAW', 'Multiscale', 'Model'], psym=[-4,-5,-3]

utplot, t, v*km_arc, utbasedata, psym=3, linestyle=1, ytit='!6Velocity (km s!U-1)', $
	/notit, /nolabel, xtickname = [' ',' ',' ',' ',' ',' ',' '], xr=[t[0]-2000,t[*]+1000], /xs, $
	yr=[-300,900], /ys, pos=[0.15,0.38,0.95,0.68];, ytickname=['-500','0','500','1000',' '] 
outplot, t, v_model*km_arc, utbasedata, psym=-3, linestyle=0
oploterror, t, v*km_arc, delta_t, v_err*km_arc, psym=3

utplot, t, a*km_arc*1000, utbasedata, psym=3, linestyle=1, /notit, ytit='!6Acceleration (m s!U-1!N s!U-1)', $
	xr=[t[0]-2000,t[*]+1000], /xs, /ys, yr=[-300,500], pos=[0.15,0.08,0.95,0.38];, ytickname=['-600','-400','-200','0','200','400',' ']
