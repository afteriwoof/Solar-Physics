...
for i=0,n_elements(clusters[0,*])-1 do begin
	...
	if clusters[0,i] lt n_elements(data[0,*]) AND clusters[1,i] lt n_elements(data[1,*]) then begin
		...
		while test1[count_test1] ge n_elements(data[1,*]) do begin
			...
			if clusters[0,test1[count_test1-1] mod n_elements(data[0,*])] ge n_elements(data[0,*]) OR $
				clusters[1,test1[count_test1-1] mod n_elements(data[1,*])] ge n_elements(data[1,*]) then step_again=1 else step_again=0
			pause
		endwhile
		if step_again eq 1 then begin
			;go through it again
		endif
