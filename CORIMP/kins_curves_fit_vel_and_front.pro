...
; Last Edited: 14-04-08

pro kins_curves_fit_vel_and_front

readcol, 'kins.txt', smj, smn, tilt, aw, hc, h, instr, date, time, h_err, delta_t, tilt_err, mag_err, tilt_err_deg, max_h_front, r_sun $
	, f='F,F,F,F,F,F,A,A,A,F,F,F,F,F,F,F'
t = anytim(date+' '+time)
utbasedata = t[0]
...
for i=12,13 do h[i] = max_h_front[i]
h_km = h*km_arc
...
h_model = (f[0]/2)*t^2 + f[1]*t*1.4 + f[2]+h[0]
v_model = f[0]*t + f[1]  
a_model = f[0]
...
utplot, t, h_rsun, utbasedata, psym=3, linestyle=1, ytit='!6Height (R!D!9n!N!X)', $
	xr=[t[0]-1000,t[*]+1000], yr=[0,16], /xs, /ys, /notit, /nolabel, $
	xtickname = [' ',' ',' ',' ',' ',' ',' '], $
...
utplot, t, v*km_arc, utbasedata, psym=3, linestyle=1, ytit='!6Velocity (km s!U-1!N)', $
	/notit, /nolabel, xtickname = [' ',' ',' ',' ',' ',' ',' '], xr=[t[0]-1000,t[*]+1000], /xs, $
	yr=[0,320], /ys, pos=[0.15,0.38,0.95,0.68];, ytickname=['-500','0','500','1000',' '] 
...
utplot, t, a*km_arc*1000, utbasedata, psym=3, linestyle=1, /notit, ytit='!6Acceleration (m s!U-1!N s!U-1!N)', $
	xr=[t[0]-1000,t[*]+1000], /xs, /ys, yr=[-60,78], pos=[0.15,0.08,0.95,0.38];, ytickname=['-600','-400','-200','0','200','400',' ']

a_model_array = replicate(a_model*km_arc[10]*1000, n_elements(t))
