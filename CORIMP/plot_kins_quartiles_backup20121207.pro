; Created	2012-08-23	from combine_pa_heights6_fit_quartiles.pro to just take in datetime, height, and position angle info.
...
pro plot_kins_quartiles, datetime, heights, pos_angles, new_heights, new_pos_angles, gather_t_nows, gather_h_nows, gather_v_nows, gather_a_nows, tog=tog, plot_quartiles=plot_quartiles, plot_median=plot_median, remove_outliers=remove_outliers, saves=saves, model=model, main_angs=main_angs, sav_gol=sav_gol, quadratic=quadratic, linear=linear
...
if keyword_set(tog) then begin
	toggle, /portrait, /color, f='plot_kins_quartiles.ps'
	!p.background=1
	!p.charsize=3.0
	!p.charthick=5
	!p.thick=3
...
for i=0,n_elements(rsun)-1 do rsun[i] = (pb0r(datetime[i],/arcsec))[2]
km_arc = 6.955e8 / (1000.*rsun)
heights_km = fltarr(n_elements(heights))
for i=0,n_elements(heights)-1 do heights_km[i] = heights[i]*km_arc[i]
...
for i=0,n_elements(datetime)-2 do begin
	if datetime[i+1] gt datetime[i] then v[i] = (heights[i+1]-heights[i])/(time[i+1]-time[i])
endfor

for i=0,n_elements(datetime)-3 do begin
...
utplot, t, (heights_km/1000.), utbasedata, psym=1, ytit='Height (Mm)', pos=[0.15,0.60,0.95,0.88],yr=[0,2.5e4],/ys, $
	xr=[t[0]-xls,t[n_elements(t)-1]+xrs], /xs, /nolabel , xtickname=[' ',' ',' ',' ',' ',' ',' '], /nodata, $
	xtit='';color=3
...
for i=min(pos_angles),max(pos_angles) do begin
	...
	if cnt gt 1 then begin
		...
		if keyword_set(linear) AND (n_elements(t_now) ge 2) then begin
                        ...
                        outplot, t_now, h_fit/1000., psym=-3
...
;		utplot, t_param, h_param, psym=2, ytit='Height (m)'
		...
		f = mpfitexpr(yf, t, h_param, /quiet)
...
utplot, t_all[0:kins_counter[0]-1], v_all[0:kins_counter[0]-1], utbasedata, psym=1, ytit='Velocity (km s!U-1!N)', $
	...
	xtickname=[' ',' ',' ',' ',' ',' ',' '], xtit='';, color=kins_color[0]
...
while flag eq 0 do begin
	...
	t_now = new_t[big_count+count]; the current time
	while new_t[big_count+count+1] gt new_t[big_count+count] do begin
		if (big_count+count+2) lt n_elements(new_t) then count += 1 else goto, jump2
		h_now = [h_now, new_heights[big_count+count]]
		t_now = [t_now, new_t[big_count+count]]
...
if keyword_set(plot_quartiles) then begin
	...
	if keyword_set(plot_median) then outplot, t_bins, v_meds, utbasedata, psym=-3, thick=1, color=c, line=0
endelse
...
while flag eq 0 do begin
        ...
        t_now = new_t[big_count+count]; the current time
        while new_t[big_count+count+1] gt new_t[big_count+count] do begin
                if (big_count+count+2) lt n_elements(new_t) then count += 1 else goto, jump3
                h_now = [h_now, new_heights[big_count+count]]
                t_now = [t_now, new_t[big_count+count]]
                if n_elements(h_now) gt 2 then begin
                        v_now = deriv((anytim(t_now)-anytim(t_now[0])), h_now)
...
if keyword_set(plot_quartiles) then begin
	...
	if keyword_set(plot_median) then outplot, t_bins, a_meds, utbasedata, psym=-3, thick=1, color=c, line=0
endelse
