...
pro find_pa_heights_masked, fls, pa_total, detection_info, det_fls, det_stack, cme_prof_fls, out_dir, plot_prof=plot_prof, loud=loud, pre_loud=pre_loud, pauses=pauses, pngs=pngs, no_plots=no_plots

if n_elements(fls) ne n_elements(det_fls) then begin
        print, 'Number of files do not match det_fls'
        in_times = strmid(file_basename(fls), 28, 15)
...
; looping over each CME detection ROI
	...
	print, '*********'
	print, 'Restoring ', cme_prof_fls[i]
	print, '*********'
	restore, cme_prof_fls[i]
	sz = size(cme_prof,/dim)
	mask = intarr(sz[0]+1,sz[1])
	mask[0:sz[0]-1,*] = smooth(cme_prof,10)
	mask[0:4,*] = 0
	...
	mask[(sz[0]-7):(sz[0]-1),*] = 0
	mask = dilate(mask,replicate(1,3,3))
	for ii=0,4 do mask[i,*] = mask[5,*]
	for ii=0,5 do mask[sz[0]-i-1,*] = mask[sz[0]-7,*]
	sz_mask = size(mask, /dim)
...
;				if keyword_set(pauses) then plots, [lowest_pt[0],next_lower_ptx], [lowest_pt[1]/100,next_lower_pty/100], psym=-2, color=5, thick=1
		...
		for test_ptx=1,n_elements(higher_ptsx)-1 do begin
			...
			help, mask, current_x, max_higher_ptsy/100
...
;						endif
		...
		if kin_count lt 10 then save, definite_x, definite_y, f=out_dir+'/cme_kin_prof_'+int2str(i)+'_00'+int2str(kin_count)+'_'+int2str(k)+'.sav'
		if kin_count ge 10 AND kin_count lt 100 then save, definite_x, definite_y, f=out_dir+'/cme_kin_prof_'+int2str(i)+'_0'+int2str(kin_count)+'_'+int2str(k)+'.sav'
		if kin_count ge 100 AND kin_count lt 1000 then save, definite_x, definite_y, f=out_dir+'/cme_kin_prof_'+int2str(i)+'_'+int2str(kin_count)+'_'+int2str(k)+'.sav'

		;save, definite_x, definite_y, f=out_dir+'/cme_kin_prof_'+int2str(i)+'_'+int2str(kin_count)+'.sav'
		print, 'Saving definite_x and definite_y as cme_kin_prof_'+int2str(i)+'_'+int2str(kin_count)+'_'+int2str(k)+'.sav'
		kin_count += 1
		...
		delvarx, higher_ptsx, current_x, max_higher_ptsy, current_col_y, prev_current_x, current_col_y, lower_ptsx_act
