...
pro kins_powerlaws, file, prominence_file, edges=edges, tog=tog, heights=heights, occulter=occulter, arc_length=arc_length, mids=mids, prominence=prominence, powerlaws=powerlaws, least_squares=least_squares, no_fits=no_fits
...
if ~keyword_set(heights) then begin
	...
	; commenting out drag	outplot, tt, vv, utbasedata, lines=2, thick=3
		...
		;print, 'First order fit to angular width'
		usersym, tempx, tempy, /fill
		if ~keyword_set(no_fits) then if ~keyword_set(powerlaws) then legend, ['!9Dq!3!D!N = '+string(f3[0],f='(D0.1)')+'(r/R'+sunsymbol()+')!U'+string(f3[1],f='(D0.1)')], $
