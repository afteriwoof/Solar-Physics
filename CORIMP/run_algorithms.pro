; Created 20110928 from run_algorithms_edges_tvscl.pro
;
...
pro run_algorithms, fls, pa_total, out_dir, saves0=saves0, show=show, pauses=pauses, no_png=no_png, no_reflect=no_reflect, gail=gail, dynamic=dynamic, model=model, stereo=stereo
...
while count lt n_elements(fls) do begin & $
	...
	in.filename = strmid(fls[count],strpos(fls[count],'/')+79,12)
	;print, 'in.filename ', in.filename
	
	if keyword_set(model) then begin
		in.filename = strmid(in.filename,strpos(in.filename,'model')+6,12)
		goto, jump3
	endif
	
;	if ~keyword_set(stereo) then begin
;                date = strmid(fls[count], strpos(fls[count],'soho.fits.gz')-19,8)
;                date += strmid(fls[count], strpos(fls[count],'soho.fits.gz')-10,6)
;       		in.filename = date
;	 endif else begin
;		date = strmid(fls[count], strpos(fls[count], 'stereo.fits.gz')-23,8)
;		date += strmid(fls[count], strpos(fls[count],'stereo.fits.gz')-14,6)
;       		in.filename = date
;	endelse
	...
	if max(cme_mask) gt max_cme_mask then begin
		...
		if ~keyword_set(gail) then begin
			...
			if ~keyword_set(no_png) then if in.i4_instr eq 'C3' || in.i4_instr eq 'c3' then x2png, out_dir+'/CME_front_edges_'+in.filename+'_C3.png'
		       	if ~keyword_set(no_png) then if in.i3_instr eq 'C2' || in.i3_instr eq 'c2' then x2png, out_dir+'/CME_front_edges_'+in.filename+'_C2.png'
			if keyword_set(pauses) then pause
		endif
		goto, jump2
	endelse
...
;		if ~keyword_set(no_png) then if in.i3_instr eq 'C2' || in.i3_instr eq 'c2' then x2png, out_dir+'/CME_front_ell_'+count_label+'_C2_tvscl.png'
	...
	;save, in, da, f=out_dir+'/in_da_'+in.filename+'.sav'
...
save, plots_list, f='plots_list_'+in.filename+'.sav'
