...
pro run_automated, fls, out_dir, pa_total, gail=gail
...
while count lt n_elements(fls) do begin
	...
	if keyword_set(gail) then in.filename = strmid(file_basename(fls[count]), str_count, 15) $
		else in.filename = strmid(file_basename(fls[count]), str_count, 15)
	...
	if ~keyword_set(no_reflect) then begin
		if ~keyword_set(stereo) then da=reflect_inner_outer(in,da) else da=reflect_inner_outer(in,da,/stereo)
	endif
