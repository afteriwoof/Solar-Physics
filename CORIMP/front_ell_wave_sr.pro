...
; Created: 19-01-09 from front_ell_kins.pro
; Last Edited: 19-01-09

; Code to take the CME measurements from the same source region as the wave analysis by Dave.

pro front_ell_wave_sr, x, y, x_sr, y_sr, errs, in, da, ang, ell_info, r_err, x_ellipse, y_ellipse, perror, max_h_front, apex_a, noflank=noflank
	...
	; Correct the coords of front for source region to be at origin
	x_map_sr = x_map - x_sr
	y_map_sr = y_map - y_sr
	...
	recpol, x_map_sr, y_map_sr, r, a, /degrees
	...
	if noplot eq 0 then begin
		...
		plots, [x_sr, mj1x], [y_sr, mj1y], line=5, color=5
		plots, [x_sr, mj2x], [y_sr, mj2y], line=5, color=5
		plots, [x_sr, mn1x], [y_sr, mn1y], line=5, color=5
		plots, [x_sr, mn2x], [y_sr, mn2y], line=5, color=5
	endif
	...
	; Take the furthest point on the ellipse from Source Region
	x_ellipse -= x_sr
	y_ellipse -= y_sr
	...
	height_apex = sqrt( (x_apex - x_sr)^2. + (y_apex - y_sr)^2. )
	print, 'height_apex(arcsec): ', height_apex
	print, 'height_apex(km): ', height_apex*725
	if noplot eq 0 then plots, [x_sr,x_apex], [y_sr,y_apex], color=3
	
	height_centre = sqrt( (p[2] - x_sr)^2. + (p[3] - y_sr)^2. )
	print, 'height_centre(arcsec): ', height_centre
	print, 'height_centre(km): ', height_centre*725
	if noplot eq 0 then plots, [x_sr,p[2]], [y_sr,p[3]], line=1, color=5
	...
	if true eq 0 then begin	
		...
		max_cone_y += y_sr
		if noplot eq 0 then begin
			plots, [x_sr,min_cone_x], [y_sr,min_cone_y]
			plots, [x_sr,max_cone_x], [y_sr,max_cone_y]
		endif
	...
	;if true eq 0 then begin
		if apex_a lt min(a_front) or apex_a gt max(a_front) then begin
			...
			temp_height_apex = sqrt( (temp_x_apex - x_sr)^2. + (temp_y_apex - y_sr)^2. )
			print, 'temp_height_apex(arcsec): ', temp_height_apex
			if noplot eq 0 then plots, [x_sr,temp_x_apex], [y_sr,temp_y_apex], color=4
		endif
	;endif else begin
