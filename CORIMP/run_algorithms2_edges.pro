; Created 17-02-11 from run_algorithms2.pro to split out for different CMEs deteced in the one image.
...
pro run_algorithms2_edges, in, da, count, multmods, cme_mask_thr, edges, contour_count, show=show, pauses=pauses
...
if keyword_set(show) then begin
	...
	print, 'plot_image, sigrange(multmask)'
	if keyword_set(pauses) then pause
endif
...
if keyword_set(show) then begin
	plot_image, multmask_binary
	print, 'plot_image, multmask_binary'
	if keyword_set(pauses) then pause
endif

print, 'Currently using edges[*,*,2] from wtmm, corresponding to array entry scale 5 of the canny_atrous'
if keyword_set(show) then begin
	plot_image, multmask_binary*edges[*,*,2]
	print, 'plot_image, multmask_binary*edges[*,*,2]'
	if keyword_set(pauses) then pause
endif
unwrap_binary = polar(multmask_binary, in.xcen, in.ycen) & $
if keyword_set(show) then begin
	plot_image, unwrap_binary
	print, 'plot_image, unwrap_binary'
	if keyword_set(pauses) then pause
endif
...
if keyword_set(show) then begin
	plot_image, multmask_binary
	print, 'plot_image, multmask_binary (radial segment removed?)'
	if keyword_set(pauses) then pause
endif
...
if keyword_set(show) then begin
	plot_image, sigrange(multmask)
	print, 'plot_image, sigrange(multmask*edges[*,*,2])'
	if keyword_set(pauses) then pause
endif
...
if keyword_set(show) then begin
	plot_image, sigrange(unwrap)
	print, 'plot_image, unwrap (the unwrapped multmask of the edges)'
	if keyword_set(pauses) then pause
endif
...
if in.i4_instr eq 'C3' then begin
	...
	if keyword_set(show) then begin
		plot_image, sigrange(multmask)
		plots, xf_new, yf_new, psym=1, color=2
		print, 'plot_image, multmask & plots, xf_new, yf_new, psym=1, color=2'
		if keyword_set(pauses) then pause
	endif
	...
	if keyword_set(show) then begin
		plots, xf_out_new, yf_out_new, psym=1, color=4
		plots, xe, ye, psym=-3, color=4
...
loadct, 0 & $
if contour_count eq 0 then plot_image, sigrange(da, /use_all) & $
...
loadct,0
if contour_count eq 0 then plot_image, da
; delete the plot_image repeat since currently issue with overplotting multiple detections
plot_image,da
...
;	plots, x_spline, y_spline, psym=-3, color=6
	plots, xe_noflanks, ye_noflanks, psym=-3, color=2
	plots, flank1x, flank1y, psym=4, color=5
	plots, flank2x, flank2y, psym=4, color=5
	oplot, [in.xcen,flank1x], [in.ycen,flank1y]
	oplot, [in.xcen,flank2x], [in.ycen,flank2y]
;	plots, xe, ye, psym=-3, color=3 & $
	for temp=0,countc-1 do begin
		plots, [in.xcen,radxs[temp]+in.xcen],[in.ycen,radys[temp]+in.ycen],linestyle=2,color=6
	endfor
	plots, ave(xy[0,*]), ave(xy[1,*]), psym=6, color=7
endif else begin
	set_line_color
	print, '-----------------'
