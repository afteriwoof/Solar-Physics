; Reads in a single front from the Matlab code (.png image) and fits ellipse

; Last Edited: 25-07-07

pro lasco_front_ell, image, in, da, height_centre, height_apex, aw, ratio, ell_tilt, semimajor, semiminor, ang
	...
	plot_helio, in.date_d$obs, roll=in.crota1, /over
	...
	if ans ne 'n' then begin
		for i=0,sz_xf[0]-1 do begin
			if x_front[i] eq 0 AND y_front[i] eq 0 then goto, jump2
			plots, x_front[i], y_front[i], psym=1
			read, 'Front? y/n', ans
			if ans ne 'n' then begin
	...
	plots, my_xfront, my_yfront, psym=1
       	x_front = my_xfront
       	y_front = my_yfront
