...
pro find_pa_heights_masked, pa_total, detection_info, plots_fls, plots_list, plot_prof=plot_prof, loud=loud, pauses=pauses, pngs=pngs
...
;	for k_count=90,detection_info[1,i] do begin
...
;		if keyword_set(pauses) then pause
		...
		if mask[lowest_pt[0],lowest_pt[1]/100] eq 0 then begin
			...
			if n_elements(indx) lt n_elements(cme_prof_ptsx) then begin
				...
				if keyword_set(loud) then print, 'goto, jump9 ' & goto, jump9
			endelse
	...
	;	pause
		...
		for test_ptx=1,n_elements(higher_ptsx)-1 do begin
			...
			if first_prev eq 0 then begin
				...
				if keyword_set(pauses) then plots, [current_x,highest_pt[0]], [max_higher_ptsy/100,highest_pt[1]/100], psym=-5, color=3, thick=1
				if keyword_set(pauses) then pause
				if where(test_arr eq 0) ne [-1] then begin
				...
				if where(test_arr eq 0) ne [-1] then begin
					...
					for check_down=0,n_elements(current_col_y)-1 do begin
						...
						if keyword_set(pauses) then pause
						if max_higher_ptsy gt current_col_y[check_down] then begin
							if keyword_set(pauses) then plots, [current_x,prev_x],[current_col_y[check_down]/100,prev_max_higher_ptsy/100],psym=-2,color=4
		...
		if keyword_set(pngs) then x2png, 'scan'+int2str(i)+'_'+int2str(k)+'_'+int2str(prof_count)+'.png'
