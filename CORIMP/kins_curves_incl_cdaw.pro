...
pro kins_curves_incl_cdaw, fit_vel=fit_vel, seeds=seeds
...
if keyword_set(fit_vel) then begin
	...
	h_model = (f[0]/2.)*t^2 + f[1]*t + f[2]+h[0]
	v_model = f[0]*t + f[1]
	a_model = f[0]
endif else begin
...
if keyword_set(seeds) then begin
	readcol, 'seeds.txt', date_seeds, time_seeds, h_seeds, f='A,A,F'
...
utplot, t, h_rsun, utbasedata, psym=3, linestyle=1, ytit='!6Height (R!D!9n!N!X)', $
	xr=[t[0]-xls,t[*]+xrs], yr=[0,22], /xs, /ys, /notit, /nolabel, $
	xtickname = [' ',' ',' ',' ',' ',' ',' '], $
...
if keyword_set(seeds) then begin
	outplot, t_seeds, h_seeds, utbasedata, psym=-5, linestyle=3
	legend, ['SEEDS','CDAW', 'Multiscale', 'Model'], psym=[5,4,1,-3], charsize=1
endif else begin
...
utplot, t, v*km_arc, utbasedata, psym=3, linestyle=1, ytit='!6Velocity (km s!U-1!N)', $
	...
	yr=[-150,950], /ys, pos=[0.15,0.38,0.95,0.68];, ytickname=['-500','0','500','1000',' '] 
...
utplot, t, a*km_arc*1000, utbasedata, psym=3, linestyle=1, /notit, ytit='!6Acceleration (m s!U-1!N s!U-1!N)', $
	xr=[t[0]-xls,t[*]+xrs], /xs, /ys, yr=[-450,300], pos=[0.15,0.08,0.95,0.38];, ytickname=['-600','-400','-200','0','200','400',' ']
