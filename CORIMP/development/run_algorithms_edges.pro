; Created 17-02-11 from run_algorithms.pro to include the wtmm.pro edge detections rather than using find_outer_peak.pro alone.
...
pro run_algorithms_edges, fls, saves0=saves0, show=show, pauses=pauses, no_png=no_png
...
if in.i3_instr eq 'C2' then max_cme_mask = 1 ; Note these have to changed for xyouts line in run_algorithms2_edges.pro
...
if keyword_set(show) then begin
	plot_image, cme_mask_dil & print, 'cme_mask_dil'
	set_line_color
...
for contour_count=0,n_elements(info)-1 do begin
	...
	if keyword_set(show) then begin
		plot_image, region_check
		print, 'plot_image, region_check'
		if keyword_set(pauses) then pause
		plot_image, region_check_unique
		print, 'plot_image, region_check_unique'
		if keyword_set(pauses) then pause
	endif
	...
	if keyword_set(show) then begin
		plot_image, region_check_dilated
		print, 'plot_image, region_check_dilated'
		if keyword_set(pauses) then pause
	endif
	region_check_dilated = morph_close(region_check_dilated,replicate(1,40,40))
	if keyword_set(show) then begin
		plot_image, region_check_dilated
		print, 'plot_image, morph_close(region_check_dilated)'
		if keyword_set(pauses) then pause
...
if cme_count gt 1 then begin
	...
	for contour_count=0,cme_count-1 do begin
		...
		if keyword_set(show) then begin
			plot_image, cme_mask_thr & print, 'plot_image, cme_mask_thr'
			if keyword_set(pauses) then pause
			plot_image, region_check_unique & print, 'plot_image, region_check_unique'
			if keyword_set(pauses) then pause
			plot_image, cme_mask_thr_unique & print, 'plot_image, cme_mask_thr_unique'
			if keyword_set(pauses) then pause
		endif
		...
		; RUN_ALGORITHMS2.pro
		print, 'Running run_algorithms2_edges.pro with cme_mask_thr_unique'
		loadct, 0
		if ~keyword_set(show) then run_algorithms2_edges, in, da, count, multmods, cme_mask_thr_unique, edges, contour_count
		if keyword_set(show) && keyword_set(pauses) then run_algorithms2_edges, in, da, count, multmods, cme_mask_thr_unique, edges, contour_count, /show, /pauses
		if keyword_set(show) && ~keyword_set(pauses) then run_algorithms2_edges,in,da,count,multmods,cme_mask_thr_unique, edges, contour_count, /show
	...
	print, 'Running run_algorithms2_edges.pro with cme_mask_thr'

	if ~keyword_set(show) then run_algorithms2_edges,in,da,count,multmods,cme_mask_thr, edges, contour_count
	if keyword_set(show) && keyword_set(pauses) then run_algorithms2_edges, in, da, count, multmods, cme_mask_thr, edges, contour_count, /show, /pauses
	if keyword_set(show) && ~keyword_set(pauses) then run_algorithms2_edges,in,da,count,multmods,cme_mask_thr, edges, contour_count, /show
