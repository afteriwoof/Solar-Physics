...
pro find_pa_heights_masked, pa_total, detection_info, plots_fls, plots_list, cme_prof_fl, out_dir, plot_prof=plot_prof, loud=loud, pre_loud=pre_loud, pauses=pauses, pngs=pngs

if n_elements(out_dir) eq 0 then out_dir='.'

window, xs=1390, ys=1390
!p.multi = [0,1,2]
...
;	for k_count=78,detection_info[1,i] do begin
...
;		if keyword_set(pauses) then pause
		...
		if keyword_set(pre_loud) then print, '*** Definite point ***'
...
;				if keyword_set(pauses) then plots, [lowest_pt[0],next_lower_ptx], [lowest_pt[1]/100,next_lower_pty/100], psym=-2, color=5, thick=1
	...
	;	pause
		...
		for test_ptx=1,n_elements(higher_ptsx)-1 do begin
			...
			jump10:
			if keyword_set(loud) then print, 'plots, current_x, max_higher_ptsy/100, psym=2, color=5, thick=3   ... starting at min'
			...
			if mask[current_x, max_higher_ptsy/100] eq 0 then begin
				...
				plots, replicate(current_x, n_elements(current_col_y[keep_ind])), current_col_y[keep_ind]/100, psym=1, color=6, thick=1
...
;			read, 'skip? ', ans
			for test_pty=0,n_elements(current_col_y)-1 do begin
				if keyword_set(loud) then print, 'plots, current_x, current_col_y[test_pty]/100, psym=6, color=5, thick=3'
...
;				if ans eq 'n' then pause
				...
				plots, current_x, max_higher_ptsy/100, psym=2, color=(test_pty mod 7)+2, thick=5
...
;						endif
					...
					for check_down=0,n_elements(current_col_y)-1 do begin
						...
						if max_higher_ptsy ne current_col_y[check_down] then begin ;used to be gt instead of ne but changed on 20111109
							...
							if where(col_test_arr eq 0) eq [-1] then begin
								...
								if keyword_set(pasues) then plots, [current_x,prev_x], [max_higher_ptsy/100,prev_max_higher_ptsy/100],psym=-2,color=5,thick=3
                                                                if keyword_set(loud) then print, 'max_higher_ptsy ', max_higher_ptsy
								if keyword_set(pauses) then pause
							endif else begin
								if keyword_set(loud) then print, 'col_test_arr: check down crosses zeroes'
								if keyword_set(loud) then print, 'plots, current_x, current_col_y[check_down]/100, psym=6, color=5, thick=2'
		...
		plots, definite_x, definite_y/100, psym=8, color=4, thick=1.5
		if keyword_set(pauses) then pause
