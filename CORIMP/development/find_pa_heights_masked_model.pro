...
;		det_list	- restored det_stack.list of the det_stack*sav
;		



pro find_pa_heights_masked_model, fls_in, pa_total, detection_info, det_fls, det_stack, cme_prof_fls, out_dir, plot_prof=plot_prof, loud=loud, pre_loud=pre_loud, pauses=pauses, pngs=pngs, no_plots=no_plots
...
print, 'Looping over ', i, ' of ', loop_end; & pause
; looping over each CME detection ROI
	...
	if sz[0]-1 gt 10 then begin
		...
		print, 'Smoothing mask by 10'
	endif else begin
		mask[0:sz[0]-1,*] = smooth(cme_prof,(sz[0]-1)*0.9)
		print, 'Smoothing mask by ', (sz[0]-1)*0.9
	endelse
	mask[0:4,*] = 0
	mask[*,0:4] = 0
	...
	count_kin_profs = intarr(detection_info[1,i]-detection_info[0,i])
...
;	for k_count=137,detection_info[1,i] do begin
		...
		if n_elements(cme_prof_ptsy) eq 0 then goto, jump8
...
;		if keyword_set(pauses) then pause
		...
		if mask[lowest_pt[0],lowest_pt[1]/100] eq 0 then begin
			...
			if n_elements(indx) lt n_elements(cme_prof_ptsx) then begin
				...
				if keyword_set(pre_loud) then print, 'goto, jump9 ' & goto, jump8
			endelse
...
;		if keyword_set(pauses) then pause
		...
		if n_elements(allindx) eq 1 then begin
			...
			if keyword_set(pre_loud) then print, 'removing lowest point'
				...
				if keyword_set(pre_loud) then print, 'goto, jump9 ' & goto, jump9
			endelse
			if keyword_set(pre_loud) then print, 'goto, jump5 ' & goto, jump5
...
;						endif
		...
		temp_hdrs = all_hdrs[definite_x]
		if all_hdrs[definite_x[0]].i3_instr eq 'C2' then datetime = all_hdrs[definite_x[0]].i3_date else datetime = all_hdrs[definite_x[0]].i4_date
		for temp=1,n_elements(definite_x)-1 do begin
			if all_hdrs[definite_x[temp]].i3_instr eq 'C2' then datetime = [datetime, all_hdrs[definite_x[temp]].i3_date] $
				else datetime = [datetime, all_hdrs[definite_x[temp]].i4_date]
		endfor
		if kin_count lt 10 then save, definite_x, definite_y, datetime, f=out_dir+'/cme_kin_prof_'+int2str(i)+'_00'+int2str(kin_count)+'_'+int2str(k)+'.sav'
		if kin_count ge 10 AND kin_count lt 100 then save, definite_x, definite_y, datetime, f=out_dir+'/cme_kin_prof_'+int2str(i)+'_0'+int2str(kin_count)+'_'+int2str(k)+'.sav'
		if kin_count ge 100 AND kin_count lt 1000 then save, definite_x, definite_y, datetime, f=out_dir+'/cme_kin_prof_'+int2str(i)+'_'+int2str(kin_count)+'_'+int2str(k)+'.sav'

		;save, definite_x, definite_y, f=out_dir+'/cme_kin_prof_'+int2str(i)+'_'+int2str(kin_count)+'.sav'
		print, 'Saving definite_x, definite_y, and datetime, as cme_kin_prof_'+int2str(i)+'_'+int2str(kin_count)+'_'+int2str(k)+'.sav'
		kin_count += 1
		...
		if keyword_set(pngs) then x2png, out_dir+'/scan'+int2str(i)+'_'+int2str(k)+'_'+int2str(prof_count)+'.png'
...
;		pause
	...
	print, 'Saving '+out_dir+'/count_kin_profs_'+int2str(i)+'.sav'
