...
get_ht_pa_2d_corimp, in.naxis1, in.naxis2, in.xcen, in.ycen, x, y, ht, pa, pix_size=pix_size
...
if in.i3_instr eq 'C2' then begin
	npa = 900.
	htra = [2.35,2.35*2];range of reflection in Rs. I go right into center just for fun.
	dht=htra[1]-htra[0]
	nref=(htra[1]-htra[0])/pix_size;approximate number of cartesian pixels within reflection depth 
endif

ht_polar=make_coordinates(nref,htra);height vector for polar array
...
htnow=2*htra[0]-htnow;modify htnow so reflection works
