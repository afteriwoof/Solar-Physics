; Created 28-06-11 from run_algorithms_edges_tvscl.pro to use correct variables for cme_model input.
;
...
pro run_algorithms_cme_model, fls, pa_total, out_dir, saves0=saves0, show=show, pauses=pauses, no_png=no_png, no_reflect=no_reflect, gail=gail, dynamic=dynamic
...
while count lt n_elements(fls) do begin & $
	...
	if keyword_set(show) then begin
		...
		if keyword_set(dynamic) then begin
			...
			cme_detection_mask_corimp_dynamic_thr, in, da, modgrad[*,*,3:6], alpgrad[*,*,3:6], cme_mask, list, /show, /no_wait;, circ_pol, cme_detect, cme_detection_frames, /show, /no_wait
		endif else begin
			print, 'Running cme_detection_mask_corimp'
			cme_detection_mask_corimp, in, da, modgrad[*,*,3:6], alpgrad[*,*,3:6], cme_mask, list, /show, /no_wait
		endelse
		list_total[*,count] = list
	endif else begin
		if keyword_set(dynamic) then begin
			print, 'Running cme_detection_mask_corimp_dynamic_thr'
			cme_detection_mask_corimp_dynamic_thr, in, da, modgrad[*,*,3:6], alpgrad[*,*,3:6], cme_mask, list;, circ_pol, cme_detect, cme_detection_frames
		endif else begin
			print, 'Running cme_detection_mask_corimp'
	...
	save, cme_mask, f='cme_mask_model.sav'	
