; Created 09-03-11 from run_algorithms_edges_tvscl.pro to run on gail and save output.
;
; Last edited: 	
...
pro run_algorithms_gail, fls, out_dir, saves0=saves0, show=show, pauses=pauses, no_png=no_png
...
;if keyword_set(show) then begin
	loadct, 0
;	cme_detection_mask_corimp, in, da, modgrad[*,*,3:6], alpgrad[*,*,3:6], circ_pol, list, cme_mask, cme_detect, cme_detection_frames, /show, /no_wait
;endif else begin
	cme_detection_mask_corimp, in, da, modgrad[*,*,3:6], alpgrad[*,*,3:6], circ_pol, list, cme_mask, cme_detect, cme_detection_frames
...
if in.i3_instr eq 'C2' then max_cme_mask = 1 ; Note these have to changed for xyouts line in run_algorithms2_edges_tvscl.pro
...
if max(cme_mask) gt max_cme_mask then begin
	...
	loadct, 0
	tvscl, sigrange(da,/use_all)
	legend, 'No CME detected', charsize=2
        if ~keyword_set(no_png) then if in.i4_instr eq 'C3' then x2png, out_dir+'/CME_front_ell_C3_'+int2str(count)+'_tvscl.png'
        if ~keyword_set(no_png) then if in.i3_instr eq 'C2' then x2png, out_dir+'/CME_front_ell_C2_'+int2str(count)+'_tvscl.png'
	goto, jump2
endelse

; Saving programming time: delete later!
save, in, da, cme_mask_thr, sz, multmods, edges, f='saves0.sav'
;stop
save_jump:
...
if keyword_set(show) then begin
	...
	set_line_color
	contour,cme_mask_dil,lev=1,/over,color=3
	if keyword_set(pauses) then pause
endif
...
;	tvscl,region_check_unique
	;pause
;	tvscl,cme_mask_thr*region_check_unique
	;pause
...
if cme_count gt 1 then begin
	...
	for contour_count=0,cme_count-1 do begin
		...
		if keyword_set(show) then begin
			...
			if keyword_set(pauses) then pause
			tvscl, region_check_unique & print, 'tvscl, region_check_unique'
			if keyword_set(pauses) then pause
			tvscl, cme_mask_thr_unique & print, 'tvscl, cme_mask_thr_unique'
			if keyword_set(pauses) then pause
		endif
		...
		; RUN_ALGORITHMS2.pro
		print, 'Running run_algorithms2_edges_tvscl.pro with cme_mask_thr_unique'
		loadct, 0
		if ~keyword_set(show) then run_algorithms2_edges_tvscl, in, da, count, multmods, cme_mask_thr_unique, edges, contour_count
		if keyword_set(show) && keyword_set(pauses) then run_algorithms2_edges_tvscl, in, da, count, multmods, cme_mask_thr_unique, edges, contour_count, /show, /pauses
		if keyword_set(show) && ~keyword_set(pauses) then run_algorithms2_edges_tvscl,in,da,count,multmods,cme_mask_thr_unique, edges, contour_count, /show
	
		legend, 'CMEs detected: '+int2str(floor(cme_count)), /bottom, /right, charsize=2
		
		if ~keyword_set(no_png) && contour_count eq cme_count-1 then if in.i4_instr eq 'C3' then x2png, out_dir+'/CME_front_ell_C3_'+int2str(count)+'_'+int2str(count_unique)+'_tvscl.png'
		if ~keyword_set(no_png) && contour_count eq cme_count-1 then if in.i3_instr eq 'C2' then x2png, out_dir+'/CME_front_ell_C2_'+int2str(count)+'_'+int2str(count_unique)+'_tvscl.png'
	...
	if ~keyword_set(show) then run_algorithms2_edges_tvscl,in,da,count,multmods,cme_mask_thr, edges, contour_count
	if keyword_set(show) && keyword_set(pauses) then run_algorithms2_edges_tvscl, in, da, count, multmods, cme_mask_thr, edges, contour_count, /show, /pauses
	if keyword_set(show) && ~keyword_set(pauses) then run_algorithms2_edges_tvscl,in,da,count,multmods,cme_mask_thr, edges, contour_count, /show
